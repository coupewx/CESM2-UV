...
! !USES:
  ...
  !integer (int_kind) :: tavg_atm_ephyto1   ! COUPE added
  !integer (int_kind) :: tavg_atm_ephyto3   ! boucher 
  !integer (int_kind) :: tavg_atm_ephyto4   ! coupe added - wait did I need to add ephyto3
...
contains
  ...
  subroutine ecosys_tavg_init(marbl_instance)
    ...
    !call define_tavg_field(tavg_atm_ephyto1,'ATM_EPHYTO1',2,long_name='E*inh of PHYTOPHAEO',units='unitless', &
  ...
  subroutine ecosys_tavg_accumulate_from_diag(i, c, bid, marbl_diags, marbl_diags_stream_cnt, tavg_ids, num_elements)
    ...
    !call accumulate_tavg_field(ATM_EPHYTO1(:,:,bid), tavg_atm_ephyto1, bid, 1) ! coupe added 
    do n=1,size(diags)
