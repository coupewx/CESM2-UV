...
!
  ...
  integer :: usr_XNO2NO2_M_ndx
  integer :: usr_NO2XNO2_M_ndx
  ...
  integer :: usr_NO_NO_ndx
  integer :: usr_NO_NO2_ndx
  
  integer :: aq_NO2_NO2_ndx
  integer :: aq_NO_NO2_ndx
...
!
    ...
    usr_N2O4_M_ndx       = get_rxt_ndx( 'usr_N2O4_M' )
 ...
 !
    tag_NO2_NO2_ndx      = get_rxt_ndx( 'tag_NO2_NO2' )
...
!lke-TS1
 ...
 !
    ...
    usr_XNO2NO2_M_ndx    = get_rxt_ndx( 'usr_XNO2NO2_M' )
    usr_NO2XNO2_M_ndx    = get_rxt_ndx( 'usr_NO2XNO2_M' )
...
!
! aqueous NOx reactions
!
    aq_NO2_NO2_ndx       = get_rxt_ndx( 'aq_NO2_NO2' )
    aq_NO_NO2_ndx        = get_rxt_ndx( 'aq_NO_NO2' )
!
! high NOx reactions :  NO + NO + O2 anf NO + NO2 + O2 reactions
!
    usr_NO_NO_ndx        = get_rxt_ndx( 'usr_NO_NO' )
    usr_NO_NO2_ndx       = get_rxt_ndx( 'usr_NO_NO2' )
...
!lke--
    ...
    if (masterproc) then
       ...
       write(iulog,'(10i5)') usr_O_O2_ndx,usr_HO2_HO2_ndx,tag_NO2_NO2_ndx,tag_NO2_NO3_ndx,usr_N2O4_M_ndx,usr_N2O5_M_ndx,tag_NO2_OH_ndx,usr_HNO3_OH_ndx &
  ...
  subroutine usrrxt( rxt, temp, tempi, tempe, invariants, h2ovmr, o2vmr, &
...
!-----------------------------------------------------------------
    ...
    real(r8), intent(in)    :: o2vmr(ncol,pver)           ! oxygen (mol/mol)
...
!TS1 species
    ...
    !-----------------------------------------------------------------------
    !        ... Aqueous phase quantities for NO + NO and NO + NO2
    !-----------------------------------------------------------------------
  real(r8), parameter            :: TMIX = 258._r8          ! Temperature threshold for mixed phase clouds (K)
  real(r8), parameter            :: LS_K1 = 1.e8_r8         ! From Lee and Schwartz (1981) (M-1 s-1)
  real(r8), parameter            :: LS_K2 = 7.52e7_r8       ! From Lee and Schwartz (1981), Kasting (1985) (M-1 s-1)
  real(r8), parameter            :: LS_E298_NO = 1.9e-3_r8  ! E298 term for Henry's Law, NO
  real(r8), parameter            :: LS_E298_NO2 = 6.4e-3_r8 ! E298 term for Henry's Law, NO2
  real(r8), parameter            :: LS_DHR_NO = 1480._r8    ! Temperature term for Henry's Law, NO
  real(r8), parameter            :: LS_DHR_NO2 = 2500._r8   ! Temperature term for Henry's Law, NO2
  real(r8)                       :: wrk, heff_no, heff_no2
...
!-----------------------------------------------------------------
!	... n2o4 + m --> no2 + no2 + m (JPL19-10)
!-----------------------------------------------------------------
       if( usr_N2O4_M_ndx > 0 ) then
          if( tag_NO2_NO2_ndx > 0 ) then
             call comp_exp( exp_fac, -6643.0_r8*tinv, ncol )
             rxt(:,k,usr_N2O4_M_ndx) = rxt(:,k,tag_NO2_NO2_ndx) * 1.69492e28_r8 * exp_fac(:)
          else
             rxt(:,k,usr_N2O4_M_ndx) = 0._r8
          end if
       end if
       if( usr_XNO2NO2_M_ndx > 0 ) then
          if( tag_NO2_NO2_ndx > 0 ) then
             call comp_exp( exp_fac, -6643.0_r8*tinv, ncol )
             rxt(:,k,usr_XNO2NO2_M_ndx) = rxt(:,k,tag_NO2_NO2_ndx) * 1.69492e28_r8 * exp_fac(:)
          else
             rxt(:,k,usr_XNO2NO3_M_ndx) = 0._r8
          end if
       end if
       if( usr_NO2XNO2_M_ndx > 0 ) then
          if( tag_NO2_NO3_ndx > 0 ) then
             call comp_exp( exp_fac, -6643.0_r8*tinv, ncol )
             rxt(:,k,usr_NO2XNO2_M_ndx) = rxt(:,k,tag_NO2_NO2_ndx) * 1.69492e28_r8 * exp_fac(:)
          else
             rxt(:,k,usr_NO2XNO2_M_ndx) = 0._r8
          end if
       end if

!-----------------------------------------------------------------
!	... n2o5 + m --> no2 + no2 + m (JPL15-10)
!-----------------------------------------------------------------
...
!-----------------------------------------------------------------
!       ... NO + NO + O2 -> 2 * NO2    High NOx, From Kastings (1985)
!-----------------------------------------------------------------
       if( usr_NO_NO_ndx > 0 ) then
          call comp_exp( exp_fac, 530._r8*tinv, ncol )
          rxt(:,k,usr_NO_NO_ndx) = 3.3e-39_r8 * exp_fac(:) * m(:,k) * o2vmr(:,k)
       end if

!-----------------------------------------------------------------
!       ... NO + NO2 + O2 -> NO2 + NO3   High NOx, From Kastings (1985)
!-----------------------------------------------------------------
       if( usr_NO_NO2_ndx > 0 ) then
          call comp_exp( exp_fac, 400._r8*tinv, ncol )
          rxt(:,k,usr_NO_NO2_ndx) = 8.0e-41_r8 * exp_fac(:) * m(:,k) * o2vmr(:,k)
       end if
...
!      
       ...
       !-----------------------------------------------------------------------
       !     ... Aqueous High NOx chemistry
       !
       ! These rates come from Lee and Schwartz (1981) and follow eqns
       ! R8 and R11:
       !
       !   R1(aq) = k1 * (Hno2)^2 * (Pno2)^2            (M/s)
       !   R1(g)  = L * R * T * R1(aq)                  (atm/s)
       !   R2(aq) = k2 * (Hno) * (Hno2) * Pno * Pno2    (M/s)
       !   R2(g)  = L * R * T * R2(aq)                  (atm/s)
       !
       ! These equations assume that Pno and Pn2o are in atm and the result
       ! is in atm/s, but the solver is in terms of molecules/cm3 and 
       ! molecules/cm3/s, so R1 and R2 need to be scaled appropriately.
       !
       ! Converting from atm to molecules/cm3
       !   n/V = P/RT
       !   P(atm) * pstd / rgas / temp(i,k) * avo / 1.e6_r8 = n/V (molecule/cm3)
       !-----------------------------------------------------------------------
       if ( aq_no2_no2_ndx > 0 .or. aq_no_no2_ndx > 0 ) then
       
         lwc(:) = cwat(:ncol,k) * invariants(:ncol,k,inv_m_ndx) * mbar(:ncol,k) / avo !PJC convert kg/kg to g/cm3
         
         do i = 1, ncol
         
           ! Only include the aqueous chemistry where the cloud water may have
           ! some liquid component.
           if (temp(i,k) .gt. TMIX) then
        
             ! Get the Henry's Law values for NO and NO2
             !
             ! NOTE: These values could be taken from the table in seq_drydep_mod,
             ! but for now just hard code them
             wrk = (t0-temp(i,k))/(t0*temp(i,k))
             
             heff_no  = LS_E298_NO * exp( LS_DHR_NO * wrk )
             heff_no2 = LS_E298_NO2 * exp( LS_DHR_NO2 * wrk )

             !-----------------------------------------------------------------------
             !     ... NO2 + NO2 --> HNO2 + HNO3
             !-----------------------------------------------------------------------
             if ( aq_no2_no2_ndx > 0 ) then
               rxt(i,k,aq_no2_no2_ndx) = lwc(i) * rgas * 1.e3_r8 / pstd * temp(i,k) * &
                  LS_K1 * heff_no2 * heff_no2 / &
                  (pstd / rgas / temp(i,k) * avo / 1.e6_r8) ! converts molec/cm3 -> atm
             end if

             !-----------------------------------------------------------------------
             !     ... NO + NO2 --> 2 * HNO2
             !-----------------------------------------------------------------------
             if ( aq_no_no2_ndx > 0 ) then
               rxt(i,k,aq_no_no2_ndx) = lwc(i) * rgas * 1.e3_r8 / pstd * temp(i,k) * &
                LS_K2 * heff_no * heff_no2 / &
                (pstd / rgas / temp(i,k) * avo / 1.e6_r8) ! converts molec/cm3 -> atm
             end if
           else
             if ( aq_no2_no2_ndx > 0 ) then
               rxt(i,k,aq_no2_no2_ndx) = 0._r8
             end if
             if ( aq_no_no2_ndx > 0 ) then
               rxt(i,k,aq_no_no2_ndx) = 0._r8
             end if
           endif
         end do
       end if  
