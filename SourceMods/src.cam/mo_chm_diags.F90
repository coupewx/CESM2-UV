module mo_chm_diags
  ...
  integer :: nox_species(3),  noy_species(28)
...
contains
  ...
  subroutine chm_diags_inti
    ...
    id_n2o4    = get_spc_ndx( 'N2O4' )
    id_n2o5    = get_spc_ndx( 'N2O5' )
    id_hono    = get_spc_ndx( 'HONO' )
...
!... NOY species
    ...
    noy_species = (/ id_n, id_no, id_no2, id_no3, id_n2o5, id_hno3, id_ho2no2, id_clono2, &
                     ...
                     id_nc4cho, id_noa, id_nterpooh, id_pbznit, id_terpnit, id_n2o4, &
                     id_hono /)
!... HOX species
...
!... counting chlorine and bromines, etc... (and total H2 species)
       if ( m == id_ch4 .or. m == id_n2o4 .or. m == id_n2o5 .or. m == id_cfc12 .or. m == id_cl2 .or. m == id_cl2o2 .or. m==id_h2o2  ) then
          wgt = 2._r8
  ...
  subroutine het_diags( het_rates, mmr, pdel, lchnk, ncol )
    ...
    do m = 1,gas_pcnst
       ...
       if (gas_wetdep_method=='MOZ') then
          ...
          if ( m == id_n2o5 .or. m == id_n2o4 ) then  ! 2 NOy molecules in N2O5
             noy_wk(:ncol) = noy_wk(:ncol) + wrk_wd(:ncol)*N_molwgt/adv_mass(m)
          endif
          if ( any(sox_species == m ) ) then
