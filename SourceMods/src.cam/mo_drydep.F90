...
contains
  ...
  subroutine dvel_inti_xactive( depvel_lnd_file, clim_soilw_file, season_wes_file )
    ...
    use chem_mods,     only : adv_mass
    use mo_chem_utls,  only : get_spc_ndx
    use seq_drydep_mod,only : drydep_method, DD_XATM, DD_XLND, drydep_do_aq_nox
...
!
    ...
    if (drydep_do_aq_nox .and. (.not. (has_dvel(no_ndx) .and. has_dvel(no2_ndx)))) then
      write(iulog,*) 'dvel_initi: drydep_do_aq_nox requires NO (', has_dvel(no_ndx), ') and NO2 (', has_dvel(no2_ndx), ') in the drydep list'
      call endrun
    end if
  ...
  subroutine drydep_xactive( ncdate, sfc_temp, pressure_sfc,  &
    ...
    use physconst,      only: tmelt, pstd, mwdry
    use seq_drydep_mod, only: drydep_method,  DD_XLND, drydep_do_aq_nox, mapping
    ...
    real(r8), parameter            :: k2 = 7.52e7_r8 ! From Lee and Schwartz (1981), Kasting (1985) (M-1 s-1)
    real(r8), parameter            :: tc1 = 1._r8    ! time constant for aqueous chemistry (s)
    real(r8), dimension(ncol)      :: pno            ! Pno (atm)
    real(r8), dimension(ncol)      :: pno2           ! Pno2 (atm)
    real(r8), dimension(ncol)      :: r1             ! R1 Aqueous rate (M s-1)
    real(r8), dimension(ncol)      :: r2             ! R2 Aqueous rate (M s-1)
    real(r8), dimension(ncol)      :: heff_no        ! Original Henry's Law Value for NO (M atm-1)
    real(r8), dimension(ncol)      :: heff_no2       ! Original Henry's Law Value for NO2 (M atm-1)
    real(r8), parameter            :: TMIX = 258._r8 ! Threshold for NO & NO2 aqueous chemistry
    real(r8), parameter            :: pstd_hpa = pstd / 100._r8
    real(r8), parameter            :: mwno = 30.01_r8    ! molecular weight of NO (g/mol)
    real(r8), parameter            :: mwno2 = 46.0055_r8 ! molecular weight of NO2 (g/mol)
    ...
    !
    ! NOTE: This is similar to what is in mo_neu_wetdep.F90; however, here the lifetime
    ! of the ocean is infinite compared to the 0.7 hr for a cloud drop.
    if (drydep_do_aq_nox) then
      
      heff_no(:) = heff(:,map_dvel(no_ndx))
      heff_no2(:) = heff(:,map_dvel(no2_ndx))

      do i = 1, ncol
       
        ! Get partial pressures in atm.
        pno(i) = mmr(i,plev,no_ndx)*(mwdry/mwno)*(pressure_sfc(i)/pstd_hpa)
        pno2(i) = mmr(i,plev,no2_ndx)*(mwdry/mwno2)*(pressure_sfc(i)/pstd_hpa)
  
        ! Calculate aqueous reaction rates
        r1(i) = k1 * (heff(i,map_dvel(no2_ndx))**2) * (pno2(i)**2) * tc1
        r2(i) = k2 * heff(i,map_dvel(no_ndx)) * pno(i) * heff(i,map_dvel(no2_ndx)) * pno2(i) * tc1
    
        ! Only include the aqueous chemistry where the cloud water may have
        ! some liquid component.
        if (sfc_temp(i) .gt. TMIX) then
        
          ! Recalculate the effective Henry's Law coefficient including
          ! the aqueous reactions.
          heff(i,map_dvel(no_ndx)) = heff(i,map_dvel(no_ndx)) + (r2(i) / pno(i))
          heff(i,map_dvel(no2_ndx)) = heff(i,map_dvel(no2_ndx)) + ((2._r8 * r1(i) + r2(i)) / pno2(i))
        end if  
      end do
    end if
