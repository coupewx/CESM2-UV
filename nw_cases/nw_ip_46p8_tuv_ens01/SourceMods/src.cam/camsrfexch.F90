...
!---------------------------------------------------------------------------
  ...
  type cam_out_t 
     ...
     real(r8) :: solld(pcols)        !
     real(r8) :: solsd(pcols)        !
     real(r8) :: uva(pcols)          ! UVA radiation (W m-2)
     real(r8) :: uvb(pcols)          ! UVB radiation (W m-2)
     real(r8) :: uvc(pcols)
     real(r8) :: par(pcols)          ! photosynthetic active radiation (W m-2)
     !real(r8) :: par_fake(pcols)          ! photosynthetic active radiation (W m-2)
     !real(r8) :: ephyto1(pcols)      ! pytoplankton photoinhibition, Phaeo (Cullen, 1992)
     !real(r8) :: ephyto2(pcols)      ! pytoplankton photoinhibition, Proro (Cullen, 1992)
     !real(r8) :: ephyto3(pcols)      ! pytoplankton photoinhibition (Boucher, 1994)
     !real(r8) :: ephyto4(pcols)      ! phytoplankton photoinhibition (Neale and Thomas, 2017)
     !real(r8) :: ephyto5(pcols)      ! cocco
     
     real(r8) :: ephyto1UVA(pcols)      ! photoinhibition, Cullen et al. (1992) Diatoms - Phaeodactylum sp. using E model
     real(r8) :: ephyto1UVB(pcols)
     real(r8) :: ephyto1UVC(pcols)

     real(r8) :: ephyto4UVA(pcols)      ! photoinhibition, Neale and Thomas (2017) Small Phytoplankton - Prochlorococcus using Emax model (thresholds reported in paper)
     real(r8) :: ephyto4UVB(pcols)
     real(r8) :: ephyto4UVC(pcols)   
     
     real(r8) :: ephyto5UVA(pcols)      ! photoinhibition, Lorenzo et al. (2019) Coccolithophores - Emiliania huxleyi using T model
     real(r8) :: ephyto5UVB(pcols)
     real(r8) :: ephyto5UVC(pcols)
 
     !real(r8) :: ephyto6UVA(pcols)      ! photoinhibition, Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
     !real(r8) :: ephyto6UVB(pcols)
     !real(r8) :: ephyto6UVC(pcols)

     real(r8) :: ephyto7UVA(pcols)      ! photoinhibition,  Neale and Thomas (2017) for Small Phytoplankton - Synechococcus using Emax model
     real(r8) :: ephyto7UVB(pcols)
     real(r8) :: ephyto7UVC(pcols)

     real(r8) :: plant71(pcols)      
...
!
    ...
    do c = begchunk,endchunk
       ...
       cam_out(c)%uvc(:)      = 0._r8
       cam_out(c)%par(:)      = 0._r8
       !cam_out(c)%par_fake(:)  = 0._r8
       !cam_out(c)%ephyto1(:)  = 0._r8
       !cam_out(c)%ephyto2(:)  = 0._r8
       !cam_out(c)%ephyto3(:)  = 0._r8
       !cam_out(c)%ephyto4(:)  = 0._r8
       !cam_out(c)%ephyto5(:)  = 0._r8
       
       cam_out(c)%ephyto1UVA(:) = 0._r8
       cam_out(c)%ephyto1UVB(:) = 0._r8
       cam_out(c)%ephyto1UVC(:) = 0._r8

       cam_out(c)%ephyto4UVA(:) = 0._r8
       cam_out(c)%ephyto4UVB(:) = 0._r8
       cam_out(c)%ephyto4UVC(:) = 0._r8

       cam_out(c)%ephyto5UVA(:) = 0._r8
       cam_out(c)%ephyto5UVB(:) = 0._r8
       cam_out(c)%ephyto5UVC(:) = 0._r8

       !cam_out(c)%ephyto6UVA(:) = 0._r8
       !cam_out(c)%ephyto6UVB(:) = 0._r8
       !cam_out(c)%ephyto6UVC(:) = 0._r8

       cam_out(c)%ephyto7UVA(:) = 0._r8
       cam_out(c)%ephyto7UVB(:) = 0._r8
       cam_out(c)%ephyto7UVC(:) = 0._r8       
       
       cam_out(c)%plant71(:) = 0._r8
...
!-----------------------------------------------------------------------
   ...
   integer :: ephyto1UVA_idx, ephyto1UVB_idx, ephyto1UVC_idx ! Cullen et al. (1992) Diatoms - Phaeodactylum sp. using E model
   integer :: ephyto4UVA_idx, ephyto4UVB_idx, ephyto4UVC_idx ! Neale and Thomas (2017) Small Phytoplankton - Prochlorococcus using Emax model (thresholds reported in paper)
   !integer :: netsw_idx ! 2/7/23 COUPE - THIS MIGHT BREAK EVERYTHING
   integer :: ephyto5UVA_idx,ephyto5UVB_idx,ephyto5UVC_idx
   !integer :: ephyto6UVA_idx, ephyto6UVB_idx, ephyto6UVC_idx ! Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
   integer :: ephyto7UVA_idx, ephyto7UVB_idx, ephyto7UVC_idx !  Neale and Thomas (2017) for Small Phytoplankton - Synechococcus using Emax model 
   integer :: plant71_idx
   ...
   real(r8), pointer :: uvc(:)
   real(r8), pointer :: par(:)
   !real(r8), pointer :: par_fake(:)
   !real(r8), pointer :: ephyto1(:)
   !real(r8), pointer :: ephyto2(:)
   !real(r8), pointer :: ephyto3(:)
   !real(r8), pointer :: ephyto4(:)
   !real(r8), pointer :: ephyto5(:)
   !real(r8), pointer :: netsw(:) ! 2/7/23 COUPE - THIS MIGHT BREAK EVERYTHING
   real(r8), pointer :: ephyto1UVA(:) !
   real(r8), pointer :: ephyto1UVB(:)
   real(r8), pointer :: ephyto1UVC(:)
   real(r8), pointer :: ephyto4UVA(:) !
   real(r8), pointer :: ephyto4UVB(:)
   real(r8), pointer :: ephyto4UVC(:)
   real(r8), pointer :: ephyto5UVA(:) !
   real(r8), pointer :: ephyto5UVB(:)
   real(r8), pointer :: ephyto5UVC(:)
   !real(r8), pointer :: ephyto6UVA(:) !
   !real(r8), pointer :: ephyto6UVB(:)
   !real(r8), pointer :: ephyto6UVC(:)
   real(r8), pointer :: ephyto7UVA(:) !
   real(r8), pointer :: ephyto7UVB(:)
   real(r8), pointer :: ephyto7UVC(:)
   real(r8), pointer :: plant71(:)
   ...
   uva_idx     = pbuf_get_index('TUV_UVA', errcode=i)
   uvb_idx     = pbuf_get_index('TUV_UVB', errcode=i)
   uvc_idx     = pbuf_get_index('TUV_UVC',errcode=i)
   par_idx     = pbuf_get_index('TUV_PARE', errcode=i)
   !par_fake_idx= pbuf_get_index('TUV_PARE', errcode=i)
   !ephyto1_idx = pbuf_get_index('TUV_PHYTOPHAEO92', errcode=i)o
   !ephyto2_idx = pbuf_get_index('TUV_PHYTOPRORO92', errcode=i)
   !ephyto3_idx = pbuf_get_index('TUV_PHYTO94', errcode=i)
   !ephyto4_idx=pbuf_get_index('TUV_PHYTO17',errcode=i)
   !ephyto5_idx=pbuf_get_index('TUV_PHYTO19',errcode=i)
   !netsw_idx = pbuf_get_index('NETSW',errcode=i) !2/7/23 COUPE - THIS MIGHT BREAK EVERYTHING OR NOT BE VALID
   ephyto1UVA_idx = pbuf_get_index('TUV_PHYTO92_UVA',errcode=i) ! Cullen et al. (1992) Diatoms - Phaeodactylum sp. using E model
   ephyto1UVB_idx = pbuf_get_index('TUV_PHYTO92_UVB',errcode=i)
   ephyto1UVC_idx = pbuf_get_index('TUV_PHYTO92_UVC',errcode=i)

   ephyto4UVA_idx = pbuf_get_index('TUV_PHYTO17_UVA',errcode=i) ! Neale and Thomas (2017) Small Phytoplankton - Prochlorococcus using Emax model
   ephyto4UVB_idx = pbuf_get_index('TUV_PHYTO17_UVB',errcode=i)
   ephyto4UVC_idx = pbuf_get_index('TUV_PHYTO17_UVC',errcode=i)

   ephyto5UVA_idx = pbuf_get_index('TUV_PHYTO19_UVA',errcode=i) ! Lorenzo et al. (2019) Coccolithophores - Emiliania huxleyi using T model
   ephyto5UVB_idx = pbuf_get_index('TUV_PHYTO19_UVB',errcode=i)
   ephyto5UVC_idx = pbuf_get_index('TUV_PHYTO19_UVC',errcode=i)

   !ephyto6UVA_idx = pbuf_get_index('TUV_PHYTOGUAN_UVA',errcode=i) ! Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
   !ephyto6UVB_idx = pbuf_get_index('TUV_PHYTOGUAN_UVB',errcode=i)
   !ephyto6UVC_idx = pbuf_get_index('TUV_PHYTOGUAN_UVC',errcode=i)

   !ephyto6UVA_idx = pbuf_get_index('TUV_PHYTO10_UVA',errcode=i) ! Guan and Gao (2010) alternative formulation for coccolithophore
   !ephyto6UVB_idx = pbuf_get_index('TUV_PHYTO10_UVB',errcode=i)
   !ephyto6UVC_idx = pbuf_get_index('TUV_PHYTO10_UVC',errcode=i)

   ephyto7UVA_idx = pbuf_get_index('TUV_PHYTO14_UVA',errcode=i) ! Neale and Thomas (2014) for Small Phytoplankton - Synechococcus using Emax model 
   ephyto7UVB_idx = pbuf_get_index('TUV_PHYTO14_UVB',errcode=i)
   ephyto7UVC_idx = pbuf_get_index('TUV_PHYTO14_UVC',errcode=i)
   plant71_idx =    pbuf_get_index('TUV_PLANT71',errcode=i) ! PLANT71

   if (uva_idx > 0) then
     call pbuf_get_field(pbuf, uva_idx, uva)
   end if
   if (uvb_idx > 0) then
     call pbuf_get_field(pbuf, uvb_idx, uvb)
   end if
   if (uvc_idx > 0) then
     call pbuf_get_field(pbuf,uvc_idx,uvc)
     end if
   if (par_idx > 0) then
     call pbuf_get_field(pbuf, par_idx, par)
   end if
   !if (par_fake_idx > 0) then
   !  call pbuf_get_field(pbuf, par_fake_idx, par_fake)
   !end if
   !if (netsw_idx > 0) then  ! 2/7/23 COUPE  THIS BLOCK IS LIABLE TO BREAK THINGS
   !  call pbuf_get_field(pbuf, netsw_idx, netsw)
   !end if
   if (ephyto1UVA_idx > 0) then
     call pbuf_get_field(pbuf, ephyto1UVA_idx, ephyto1UVA)
     ! write(*,*) "does this happen",ephyto1UVA_idx
   end if
   if (ephyto1UVB_idx > 0) then
     call pbuf_get_field(pbuf, ephyto1UVB_idx, ephyto1UVB)
   end if
   if (ephyto1UVC_idx > 0) then
     call pbuf_get_field(pbuf, ephyto1UVC_idx, ephyto1UVC)
   end if

   if (ephyto4UVA_idx > 0) then
     call pbuf_get_field(pbuf, ephyto4UVA_idx, ephyto4UVA)
     ! write(*,*) "does this happen",ephyto4UVA_idx
   end if
   if (ephyto4UVB_idx > 0) then
     call pbuf_get_field(pbuf, ephyto4UVB_idx, ephyto4UVB)
   end if
   if (ephyto4UVC_idx > 0) then
     call pbuf_get_field(pbuf, ephyto4UVC_idx, ephyto4UVC)
   end if


   if (ephyto5UVA_idx > 0) then
     call pbuf_get_field(pbuf, ephyto5UVA_idx, ephyto5UVA)
   end if
   if (ephyto5UVB_idx > 0) then
     call pbuf_get_field(pbuf, ephyto5UVB_idx, ephyto5UVB)
   end if
   if (ephyto5UVC_idx > 0) then
     call pbuf_get_field(pbuf, ephyto5UVC_idx, ephyto5UVC)
   end if

   !if (ephyto6UVA_idx > 0) then
   !  call pbuf_get_field(pbuf, ephyto6UVA_idx, ephyto6UVA)
   !end if
   !if (ephyto6UVB_idx > 0) then
   !  call pbuf_get_field(pbuf, ephyto6UVB_idx, ephyto6UVB)
   !end if
   !if (ephyto6UVC_idx > 0) then
   !  call pbuf_get_field(pbuf, ephyto6UVC_idx, ephyto6UVC)
   !end if

   if (ephyto7UVA_idx > 0) then
     call pbuf_get_field(pbuf, ephyto7UVA_idx, ephyto7UVA)
   end if
   if (ephyto7UVB_idx > 0) then
     call pbuf_get_field(pbuf, ephyto7UVB_idx, ephyto7UVB)
   end if
   if (ephyto7UVC_idx > 0) then
     call pbuf_get_field(pbuf, ephyto7UVC_idx, ephyto7UVC)
   end if
   if (plant71_idx > 0) then
     call pbuf_get_field(pbuf, plant71_idx, plant71)
   end if
   ...
   do i=1,ncol
     ...
     !if (par_fake_idx > 0) then
     !  cam_out%par_fake(i)     =cam_out%netsw(i)! netsw(i)!par_fake(i)
     !end if
     !if (ephyto1_idx > 0) then
     !  cam_out%ephyto1(i) = ephyto1(i)
     !end if
     !if (ephyto2_idx > 0) then
     !  cam_out%ephyto2(i) = ephyto2(i)
     !end if
     !!if (ephyto3_idx > 0) then
     !!  cam_out%ephyto3(i) = ephyto3(i)
     !!end if
     !if (ephyto4_idx > 0) then
     !  cam_out%ephyto4(i) = ephyto4(i)
     !end if
     !if (ephyto5_idx > 0) then
     !  cam_out%ephyto5(i) = ephyto5(i)
     !end if

     if (ephyto1UVA_idx > 0) then
       cam_out%ephyto1UVA(i) = ephyto1UVA(i)
     end if
     if (ephyto1UVB_idx > 0) then
       cam_out%ephyto1UVB(i) = ephyto1UVB(i)
     end if
     if (ephyto1UVC_idx > 0) then
       cam_out%ephyto1UVC(i) = ephyto1UVC(i)
     end if

     if (ephyto4UVA_idx > 0) then
       cam_out%ephyto4UVA(i) = ephyto4UVA(i)
       !if (ephyto4UVA(i) >0) then ! JLC COUPE ADDED THIS ON 12/9/24 TO DEBUG ZEROS IN CULLEN BWF
       !        write(*,*) "ephyto4UVA is not zero" ! if this goes off then the issue is later on 
       !endif
     end if
     if (ephyto4UVB_idx > 0) then
       cam_out%ephyto4UVB(i) = ephyto4UVB(i)
     end if
     if (ephyto4UVC_idx > 0) then
       cam_out%ephyto4UVC(i) = ephyto4UVC(i)
     end if

     if (ephyto5UVA_idx > 0) then
       cam_out%ephyto5UVA(i) = ephyto5UVA(i)
     end if
     if (ephyto5UVB_idx > 0) then
       cam_out%ephyto5UVB(i) = ephyto5UVB(i)
     end if
     if (ephyto5UVC_idx > 0) then
       cam_out%ephyto5UVC(i) = ephyto5UVC(i)
     end if

     !if (ephyto6UVA_idx > 0) then
     !  cam_out%ephyto6UVA(i) = ephyto6UVA(i)
     !end if
     !if (ephyto6UVB_idx > 0) then
     !  cam_out%ephyto6UVB(i) = ephyto6UVB(i)
     !end if
     !if (ephyto6UVC_idx > 0) then
     !  cam_out%ephyto6UVC(i) = ephyto6UVC(i)
     !end if

     if (ephyto7UVA_idx > 0) then
       cam_out%ephyto7UVA(i) = ephyto7UVA(i)
     end if
     if (ephyto7UVB_idx > 0) then
       cam_out%ephyto7UVB(i) = ephyto7UVB(i)
     end if
     if (ephyto7UVC_idx > 0) then
       cam_out%ephyto7UVC(i) = ephyto7UVC(i)
     end if
     if (plant71_idx > 0) then
       cam_out%plant71(i) = plant71(i)
     end if
