...
! Namelist variables
  ...
  logical :: radheat_limit_dt = .false. ! limit the temperature change in a timestep to 1/2 of the total T 
...
!===============================================================================
  ...
  subroutine radheat_readnl(nlfile)
    ...
    namelist /radheat_nl/ nlte_use_mo, nlte_limit_co2, radheat_limit_dt
    ...
    call mpi_bcast (radheat_limit_dt, 1, mpi_logical, masterprocid, mpicom, ierr)
...
! Local variables
    ...
    integer  :: dtime                               ! timestep size (sec)
    ...
    real(r8) :: hr_lim                              ! Heating rate limit for large perturbation runs.
...
!   Merge cam long wave heating for lower atmosphere with M/LT (nlte) heating
    ...
    ! Limit heating rate so that dT can only be half the total T at the end
    ! of the timestep. This prevents it from driving the temperature negative in one
    ! time step during large perturbation experiments.
    !
    ! NOTE: It isn't clear if this is still necessary after the RRTMG fixes;
    ! however, if this is kept it should be namelist controlled. It should also
    ! be added to the radheat for CAM for consistency.
    if (radheat_limit_dt) then
      dtime = get_step_size()

      do k = 1, pver
        do i = 1, ncol
          !hr_lim = state%t(i,k) * cpair / dtime / 3._r8
          ! try to stablize... --shixiongh
          hr_lim = state%t(i,k) * cpair / dtime / 4._r8
          if (abs(ptend%s(i,k)) > hr_lim) ptend%s(i,k) = sign(hr_lim, ptend%s(i,k))
!          hr_lim = state%t(i,k) + ptend%s(i,k)*dt/1004._r8
!          if (hr_lim < 0._r8) ptend%s(i,k) = (-1._r8 * state%t(i,k)) / (dt/cpairv_loc(i,k,state%lchnk))
        end do
      end do
    end if
