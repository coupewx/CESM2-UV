...
module carma_model_mod
  ...
  ! india pakistan targeting code
    integer                             :: carma_emis_nLevs        ! number of levels
      integer                             :: carma_emis_nTargets     ! number of targets
        real(r8), allocatable, dimension(:,:) :: carma_emis_tgtSmoke   ! target smoke emission (g)
          real(r8), allocatable, dimension(:) :: carma_emis_tgtSurface   ! target smoke deposition (g)
            real(r8), allocatable, dimension(:) :: carma_emis_tgtLev       ! target levels (hPa)
              real(r8), allocatable, dimension(:) :: carma_emis_tgtLat       ! target latitudes (deg)
                real(r8), allocatable, dimension(:) :: carma_emis_tgtLon       ! target longitude (deg)
                  integer, allocatable, dimension(:)  :: carma_emis_tgtDate      ! target day (offset from start date)
                    integer, allocatable, dimension(:)  :: carma_emis_tgtTime      ! target time of day (s)
...
contains
  ...
  !!  @version Jul-2018 
  !!  @author  Chuck Bardeen 
subroutine interp1d(inN, inVal, inData, outN, outVal, outData)
    implicit none

    integer, intent(in)               :: inN
    real(kind=f), intent(in)          :: inVal(inN)
    real(kind=f), intent(in)          :: inData(inN)
    integer, intent(in)               :: outN
    real(kind=f), intent(in)          :: outVal(outN)
    real(kind=f), intent(out)         :: outData(outN)

    integer                           :: i, j
    integer                           :: inMIn, inMax, inIncr
    integer                           :: outMIn, outMax, outIncr

    if (inVal(1) .lt. inVal(2)) then
      inMin  = 1
      inMax  = inN
      inIncr = 1
    else
   ...
   do i = outMin, outMax, outIncr

      ! Check for extrapolation. Set to 0.
      if (outVal(i) .lt. inVal(inMin)) then
        outData(i) = 0._r8
        cycle
      end if

      if (outVal(i) .gt. inVal(inMax)) then
        outData(i) = 0._r8
        cycle
      end if

      ! Check for the edges. Set to the edge.
      if (outVal(i) .eq. inVal(inMin)) then
        outData(i) = inData(inMin)
        cycle
      end if

      if (outVal(i) .eq. inVal(inMax)) then
        outData(i) = inData(inMax)
        cycle
      end if

      ! Interpolate in the interior
      do while ((outVal(i) .ge. inVal(j)) .and. (j .ne. inMax))
        j = j + inIncr
      end do

      outData(i) = inData(j-inIncr) + (inData(j) - inData(j-inIncr)) * (outVal(i) - inVal(j-inIncr)) / (inVal(j) - inVal(j-inIncr))
  ...
  subroutine CARMA_DefineModel(carma, rc)
    ...
    if (masterproc) then
      ...
      carma_emis_soot = 0.0 ! we are going to force this to be zero as a test, let india-pakistan code work
...
;    refidx(:) = (1.53_f, 0.008_f)
  ...
  subroutine CARMA_EmitGas(carma, igas, icnst, dt, state, cam_in, tendency, surfaceFlux, rc)
    ...
    integer      :: ilat
    integer      :: ilon
    !india pakistan targeting variables
    integer      :: itgt                    ! target index
...
!        ((ncsec >= carma_emis_starttime) .and. (ncsec < (carma_emis_starttime + dt)))) then
  ...
  subroutine CARMA_EmitParticle(carma, ielem, ibin, icnst, dt, state, cam_in, tendency, surfaceFlux, rc)
    ...
    integer      :: itgt                    ! target index
    real(r8)     :: tlon
    real(r8)     :: colarea                 ! column area (m2)
    real(r8)     :: massfluxtgt(pver)          ! emission mass flux (kg/m2/s)
    ...
    ! Determine grid spacings. Assumes uniform grid.
    dlon = 360._r8 / plon * DEG2RAD
    dlat = 180._r8 / (plat-1) * DEG2RAD
...
! BELOW IS THE US-RUSSIA EMISSION
   if (shortname == "CRSOOT" .and. carma_emis_soot .GT. 0._f) then
      !write(*,*) 'US RUSSIA EMISSION IS OCCURRING'
...
! INDIA PAKISTAN TARGETING CODE EMISSIONS
    if (shortname == "CRSOOT" .and. carma_emis_soot .LT. 1._f) then
      ! write(*,*) 'INDIA-PAKISTAN EMISSION IS OCCURRING'
      ! Check to see if any of the targets should emit during this timestep
      !
      ! NOTE: carma_emis_stopdate provides a quick check to determine
      ! whether there is any need to check the target list; however, the
      ! user must make sure that the stop date is large enough to handle
      ! span of the targetlist.
      !
      ! NOTE: This logic won't work if the date wraps around the end of the year.
      if ((carma_emis_startdate .le. currentDate) .and. (carma_emis_stopdate .ge. currentDate)) then
        do itgt = 1, carma_emis_nTargets
          ! debug to see what carma_emis_tgtDate(itgt) is, maybe something is wrong
          if (((carma_emis_startdate + carma_emis_tgtDate(itgt)) .eq. currentDate) .and. &
              ((carma_emis_tgtTime(itgt) .ge. ncsec) .and. (carma_emis_tgtTime(itgt) .lt. (ncsec + dt)))) then
             write(*,*) 'model date is', currentDate
             write(*,*) 'target is ',itgt
            ! Loop over all of the columns.
            do icol = 1, ncol

              ! grid lons are 0-360, but specification can be -180 to +180, so convert lons
              tlon =  carma_emis_tgtLon(itgt)
              if (tlon .lt. 0._r8) tlon = 360._r8 + tlon
              tlon = tlon*DEG2RAD

              ! Determine if this column is in the range of the regions.
              !
              ! NOTE: May not handle the pole points.
              if (((lat(icol) + dlat/2._r8) .ge. carma_emis_tgtLat(itgt)*DEG2RAD) .and. &
                  ((lat(icol) - dlat/2._r8) .lt. carma_emis_tgtLat(itgt)*DEG2RAD) .and. &
                  ((lon(icol) + dlon/2._r8) .ge. tlon) .and. &
                  ((lon(icol) - dlon/2._r8) .lt. tlon)) then

                colarea = ((REARTH / 100._r8)**2) * abs(sin(lat(icol) - dlat/2._r8) - sin(lat(icol) + dlat/2._r8)) * dlon

                ! Add deposition of soot that rained to the surface.
!                surfaceFlux(icol) = surfaceFlux(icol) + carma_emis_tgtSurface(itgt) / 1000._r8 / colarea / dt

                ! The vertical distribution is from the target file, so interpolate
                ! from that to the model leves, and normalize this so that the
                ! vertical weights add to 1.
                call interp1d(carma_emis_nLevs, carma_emis_tgtLev(:) * 100._r8, carma_emis_tgtSmoke(:, itgt), pver, state%pmid(icol,:), vfunc(:))
                vfunc(:) = vfunc(:) / sum(vfunc(:))

                ! Add smoke emissions (kg/kg/s) to the smoke already emitted into this grid box
                ! from this timestep.
                massfluxtgt(:) = (sum(carma_emis_tgtSmoke(:, itgt)) / 1000._r8) * vfunc(:) / &
                  colarea / (state%pdel(icol, :) / gravit)

                tendency(icol, :) = tendency(icol, :) + (massfluxtgt(:) * carma_sootbinfactor(ibin) / dt)
...
!    end if
  ...
  subroutine CARMA_InitializeModel(carma, lq_carma, rc)
    ...
    integer                            :: tgt_did               ! target dimension id
    integer                            :: tgt_vid               ! target variable id
    integer                            :: date_vid              ! date variable
    integer                            :: time_vid              ! time variable
    integer                            :: lat_vid               ! lat variable
    integer                            :: lon_vid               ! lon variable
    integer                            :: smoke_vid             ! smoke variable
    integer                            :: surface_vid           ! deposition variable
    integer                            :: start(2)
    integer                            :: count(2)
    integer                            :: tmp
  ...
  1 format(i3,5x,i3,4x,e10.3,4x,e10.3) 
    ...
    if (masterproc) then
      ...
      if (do_print) write(LUNOPRT,*) 'carma_init(): Reading targets from ', efile
      ...
      call wrap_inq_dimid(fid, "target", tgt_did)
      call wrap_inq_dimlen(fid, tgt_did, carma_emis_nTargets)
    endif

#if ( defined SPMD )
    call mpibcast(carma_emis_nLevs, 1, mpiint, 0, mpicom)
    call mpibcast(carma_emis_nTargets, 1, mpiint, 0, mpicom)
#endif


    allocate(carma_emis_tgtLev(carma_emis_nLevs))
    allocate(carma_emis_tgtLat(carma_emis_nTargets))
    allocate(carma_emis_tgtLon(carma_emis_nTargets))
    allocate(carma_emis_tgtDate(carma_emis_nTargets))
    allocate(carma_emis_tgtTime(carma_emis_nTargets))
    allocate(carma_emis_tgtSurface(carma_emis_nTargets))
    allocate(carma_emis_tgtSmoke(carma_emis_nLevs, carma_emis_nTargets))
    ...
    if (masterproc) then
      ...
      ! Read in the tables.
      call wrap_inq_varid(fid, 'date', date_vid)
      call wrap_get_var_int(fid, date_vid, carma_emis_tgtDate)

      call wrap_inq_varid(fid, 'time', time_vid)
      call wrap_get_var_int(fid, time_vid, carma_emis_tgtTime)

      call wrap_inq_varid(fid, 'lev', lev_vid)
      call wrap_get_var_realx(fid, lev_vid, carma_emis_tgtLev)

      call wrap_inq_varid(fid, 'lat', lat_vid)
      call wrap_get_var_realx(fid, lat_vid, carma_emis_tgtLat)

      call wrap_inq_varid(fid, 'lon', lon_vid)
      call wrap_get_var_realx(fid, lon_vid, carma_emis_tgtLon)

      call wrap_inq_varid(fid, 'smokeDeposition', surface_vid)
      call wrap_get_var_realx(fid, surface_vid, carma_emis_tgtSurface)

      start(1) = 1
      count(1) = carma_emis_nLevs
      start(2) = 1
      count(2) = carma_emis_nTargets
      call wrap_inq_varid(fid, 'smoke', smoke_vid)
!      call wrap_get_vara_realx(fid, smoke_vid, start, count, carma_emis_tgtSmoke)

      ret = nf90_get_var(fid, smoke_vid, carma_emis_tgtSmoke, start, count)
      if (ret/=NF90_NOERR) then
        write(LUNOPRT,*)'WRAP_GET_VARA_REALX: error reading varid =', smoke_vid
        call endrun("CARMA_InitializeModel: CARMA_Get failed.")
      end if
      ...
      if (do_print) then
        write(LUNOPRT,*) 'nLevs = ', carma_emis_nLevs
        write(LUNOPRT,*) 'nTargets = ', carma_emis_nTargets
        write(LUNOPRT,*) ''
        write(LUNOPRT,*) 'target, date, time, lat, lon, deposition (g), smoke (g)'
        do itgt = 1, carma_emis_nTargets
          write(LUNOPRT,*) itgt, carma_emis_tgtDate(itgt), carma_emis_tgtTime(itgt), carma_emis_tgtLat(itgt), carma_emis_tgtLon(itgt), carma_emis_tgtSurface(itgt), sum(carma_emis_tgtSmoke(:,itgt))
        end do

        write(LUNOPRT,*) 'carma_init(): Total Deposition = ', sum(carma_emis_tgtSurface)
        write(LUNOPRT,*) 'carma_init(): Total Emission = ', sum(carma_emis_tgtSmoke)
        write(LUNOPRT,*) 'carma_init(): Done with emission table.'
      end if
    endif

#if ( defined SPMD )
    call mpibcast(carma_emis_tgtDate,  carma_emis_nTargets, mpiint, 0, mpicom)
    call mpibcast(carma_emis_tgtTime,  carma_emis_nTargets, mpiint, 0, mpicom)
    call mpibcast(carma_emis_tgtLev,   carma_emis_nLevs, mpir8, 0, mpicom)
    call mpibcast(carma_emis_tgtLat,   carma_emis_nTargets, mpir8, 0, mpicom)
    call mpibcast(carma_emis_tgtLon,   carma_emis_nTargets, mpir8, 0, mpicom)
    call mpibcast(carma_emis_tgtSurface, carma_emis_nTargets, mpir8, 0, mpicom)
    call mpibcast(carma_emis_tgtSmoke, carma_emis_nTargets*carma_emis_nLevs, mpir8, 0, mpicom)
#endif
