...
!=============================================================================*
      ...
      public :: rdxs_init
      public :: no2xs_a, no2xs_b, o2_xs, so2_xs, co2_xs, hbr_xs, hcl_xs, h2o_xs, no_xs
      ...
      real(rk), protected, allocatable :: co2_xs(:)
      real(rk), protected, allocatable :: hbr_xs(:)
      real(rk), protected, allocatable :: hcl_xs(:)
      real(rk), protected, allocatable :: h2o_xs(:)
      real(rk), protected, allocatable :: no_xs(:)
    ...
    CONTAINS
      ...
      if (.not. allocated(co2_xs) ) then
         allocate(co2_xs(nw),stat=astat)
         co2_xs = xnan
         istat = istat + astat
      endif
      if (.not. allocated(hbr_xs) ) then
         allocate(hbr_xs(nw),stat=astat)
         hbr_xs = xnan
         istat = istat + astat
      endif
      if (.not. allocated(hcl_xs) ) then
         allocate(hcl_xs(nw),stat=astat)
         hcl_xs = xnan
         istat = istat + astat
      endif
      if (.not. allocated(h2o_xs) ) then
         allocate(h2o_xs(nw),stat=astat)
         h2o_xs = xnan
         istat = istat + astat
      endif
      if (.not. allocated(no_xs) ) then
         allocate(no_xs(nw),stat=astat)
         ! NOTE: Don't currently have cross section for NO, so
         ! zero it out.
         no_xs = 0._rk
         istat = istat + astat
      endif
...
!_______________________________________________________________________
      ...
      call rdco2xs(nw+1,wl,co2_xs, errmsg, errflg)
      call rdhbrxs(nw+1,wl,hbr_xs, errmsg, errflg)
      call rdhclxs(nw+1,wl,hcl_xs, errmsg, errflg)
      call rdh2oxs(nw+1,wl,h2o_xs, errmsg, errflg)
...
!=============================================================================*
      ...
      SUBROUTINE rdco2xs(nw,wl,co2xs, errmsg, errflg)
!-----------------------------------------------------------------------------*
!=  PURPOSE:                                                                 =*
!=  Read SO2 molecular absorption cross section.  Re-grid data to match      =*
!=  specified wavelength working grid.                                       =*
...
!! local:
      ...
      errflg = 0
!************ absorption cross sections:
! Recommended CO2 absorption cross sections in the wavelength range 58.0-167.0 nm taken
! from Nakata, R. S., K. Watanabe, F. M. Matsunaga, Sci. Light, 14, 54, 1965.
! Between 166.0-199.0nm the compiled cross section from Huffman, 1971.

      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/CO2/XS_CO2.txt',STATUS='old')
      DO i = 1,20 
        read(kin,*)
      ENDDO
      n = 55 
      ...
      CALL addpnt(x1,y1,kdata,n,      1.e+38_rk,0._rk,errmsg, errflg)
      if (errflg.ne.0) return
      CALL inter2(nw,wl,co2xs,n,x1,y1,errmsg, errflg)

      END SUBROUTINE rdco2xs

      SUBROUTINE rdhbrxs(nw,wl,hbrxs, errmsg, errflg)
!-----------------------------------------------------------------------------*
!=  PURPOSE:                                                                 =*
!=  Read HBR molecular absorption cross section.  Re-grid data to match      =*
!=  specified wavelength working grid.                                       =*
!-----------------------------------------------------------------------------*
...
!! local:
      ...
      errflg = 0
!************ absorption cross sections:
! Recommended HBR absorption cross section from:
! S.P. Sander, R.R. Friedl, D.M. Golden, M.J. Kurylo, G.K. Moortgat, H. Keller-Rudek, 
! P.H. Wine, A.R. Ravishankara, C.E. Kolb, M.J. Molina, B.J. Finlayson-Pitts, R.E. Huie, 
! V.L. Orkin, “Chemical Kinetics and Photochemical Data for Use in Atmospheric Studies. 
! Evaluation Number 15“, JPL Publication 06-2, Jet Propulsion Laboratory, Pasadena, 2006.


      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/HBR/XS_HBR_JPL06.txt',STATUS='old')
      DO i = 1,44 
        read(kin,*)
      ENDDO
      n = 40 
      ...
      CALL addpnt(x1,y1,kdata,n,      1.e+38_rk,0._rk,errmsg, errflg)
      if (errflg.ne.0) return
      CALL inter2(nw,wl,hbrxs,n,x1,y1,errmsg, errflg)

      END SUBROUTINE rdhbrxs

      SUBROUTINE rdhclxs(nw,wl,hclxs, errmsg, errflg)
!-----------------------------------------------------------------------------*
!=  PURPOSE:                                                                 =*
!=  Read HCL molecular absorption cross section.  Re-grid data to match      =*
!=  specified wavelength working grid.                                       =*
!-----------------------------------------------------------------------------*
...
!! local:
      ...
      errflg = 0
!************ absorption cross sections:
! Recommended HCL absorption cross section from:
! S.P. Sander, R.R. Friedl, D.M. Golden, M.J. Kurylo, G.K. Moortgat, H. Keller-Rudek, 
! P.H. Wine, A.R. Ravishankara, C.E. Kolb, M.J. Molina, B.J. Finlayson-Pitts, R.E. Huie, 
! V.L. Orkin, “Chemical Kinetics and Photochemical Data for Use in Atmospheric Studies. 
! Evaluation Number 15“, JPL Publication 06-2, Jet Propulsion Laboratory, Pasadena, 2006.
      ...
      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/HCL/XS_HCL_JPL06.txt',STATUS='old')
      DO i = 1,22 
        read(kin,*)
      ENDDO
      n = 31 
      ...
      CALL addpnt(x1,y1,kdata,n,      1.e+38_rk,0._rk,errmsg, errflg)
      if (errflg.ne.0) return
      CALL inter2(nw,wl,hclxs,n,x1,y1,errmsg, errflg)

      END SUBROUTINE rdhclxs
      
      SUBROUTINE rdh2oxs(nw,wl,h2oxs, errmsg, errflg)
!-----------------------------------------------------------------------------*
!=  PURPOSE:                                                                 =*
!=  Read H2O molecular absorption cross section. This is the sum of three    =*
!=  different crossections (JPL, Cantrell, Yoshino) for different products.  =*
!= Re-grid data to match specified wavelength working grid.                  =*
!-----------------------------------------------------------------------------*
...
!************ absorption cross sections:
! Recommended H2O absorption cross section for:
!   H2O + hv -> H + OH
! from:
...
!   page 117
      ...
      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/H2O/XS_H2O_JPL06.txt',STATUS='old')
      DO i = 1,28 
        read(kin,*)
      ENDDO
      n = 8 
      ...
      CALL addpnt(x1,y1,kdata,n,      1.e+38_rk,0._rk,errmsg, errflg)
      if (errflg.ne.0) return
      CALL inter2(nw,wl,h2oxs,n,x1,y1,errmsg, errflg)
      
!************ absorption cross sections:
! Recommended H2O absorption cross section for:
!   H2O + hv -> H2 + O(1D)
! from:
!   Cantrell, Zimmer and Tyndall
!   Absorption cross sections for water vapor from 183 to 193nm
!   Geophys. Res. Lett., 24, 17, 2195-2198, 1997.

      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/H2O/XS_H2O_cantrell_1996.txt',STATUS='old')
      DO i = 1,12
        read(kin,*)
      ENDDO
      n = 11 
      ...
      CALL addpnt(x1,y1,kdata,n,      1.e+38_rk,0._rk,errmsg, errflg)
      if (errflg.ne.0) return
      CALL inter2(nw,wl,tmp,n,x1,y1,errmsg, errflg)
      h2oxs = h2oxs + tmp
...
!   H2O + hv -> 2H + O(3P)
! from:
!   ABSORPTION CROSS SECTION MEASUREMENTS OF WATER VAPOR IN THE WAVELENGTH 
!   REGION 120 TO 188 NM K. Yoshino, J.R. Esmond, W.H. Parkinson 
!   Harvard-Smithsonian Center for Astrophysics, 60 Garden Street, 
!   Cambridge, MA 02138, U.S.A. K. Ito, T. Matsui Photon Factory, 
!   National Laboratory of High Energy Physics, Tsukuba, Ibaruki, 
!   305, Japan Chemical Physics, 211, pp. 387-391, 1996 

      OPEN(UNIT=kin,FILE=trim(input_data_root)//'/DATAE1/H2O/XS_H2O_yoshino_1996.txt',STATUS='old')
      DO i = 1,23
        read(kin,*)
      ENDDO
      n = 6783 
