module mo_imp_sol
  ...
  !higher iteration makes solver correct but maybe less unstable, our case crashed at 20th day
  !lets try the previous config for Jan...
   integer, parameter :: itermax = 22
   integer, parameter :: cut_limit = 10
          ...
          iter_loop : do nr_iter = 1,itermax
             ...
             do m = 1,clscnt4
                do i = 1, avec_len
                   ...
                   if (solution_blk(i,m) < 1.0e-30_r8) then
                      forcing_blk(i,m) = 0.0_r8
                      solution_blk(i,m) = 0.0_r8
                   end if
                end do
             end do
...
!                      write(iulog,'(1x,a16,1x,1pe10.3)') solsym(clsmap(m,4)), max_delta(m)
       ...
       if( ofl > chnkpnts ) then
          exit chnkpnts_loop
                   ...
                   do m = 1,gas_pcnst
                      do i = 1,ncol*nlev
                         if (shr_infnan_isnan(base_sol(i,m))) then
!                            write(*,*) "2 infnan base_sol gas ", m, "avec_len ", i
!                            write(*,*) "solval ", base_sol(i,m), "solval prev ", base_sol(i-1,m)
                            base_sol(i,m) = base_sol(i-1,m)
!                            base_sol(i,m) = 0.0_r8
                         end if
                         if (base_sol(i,m) > 1.0_r8) then
!                            write(*,*) "2 big base_sol gas ", m, "avec_len ", i
!                            write(*,*) "solval ", base_sol(i,m), "solval prev ", base_sol(i-1,m)
                            base_sol(i,m) = base_sol(i-1,m)
!                            base_sol(i,m) = 0.0_r8
                         end if
                         if (base_sol(i,m) < 0.0_r8) then
!                            write(*,*) "2 small base_sol gas ", m, "avec_len ", i
!                            write(*,*) "solval ", base_sol(i,m), "solval prev ", base_sol(i-1,m)
                            base_sol(i,m) = base_sol(i-1,m)
!                            base_sol(i,m) = 0.0_r8
                         end if
