...
!-----------------------------------------------------------------------
      ...
      use radxfr_cam,        only : radxfr_cam_update, actinic_fluxes, do_tuv_photo, do_tuv_cloud, do_actinic_so2 ,do_actinic_co2, do_actinic_h2o, do_actinic_hbr, do_actinic_hcl
...
!         cldw = 0._r8
         ...
         if (do_actinic_so2) then   
            if (id_so2>0) then
               ...
               call endrun('mo_waccm_hrates: must include SO2 for radxfr_do_so2')
            end if
         else
            so2_radxfr(:,:) = 0._r8
         end if     
