...
!
    ...
    use cam_abortutils,    only : endrun
    use radxfr_cam,        only : radxfr_cam_update, do_tuv_cloud, do_actinic_so2  ,do_actinic_co2, do_actinic_h2o, do_actinic_hbr, do_actinic_hcl
    ...
    real(r8), dimension(ncol,pver) :: &
         ...
         o2vmr, &                                          ! oxygen volume mixing ratio
  ...
  ! for aerosol formation....  
    ...
    real(r8) :: co24rad(ncol,pver)                            ! co2 for radxfr (vmr)
    real(r8) :: h2o4rad(ncol,pver)                            ! h2o for radxfr (vmr)
    real(r8) :: hbr4rad(ncol,pver)                            ! hbr for radxfr (vmr)
    real(r8) :: hcl4rad(ncol,pver)                            ! hcl for radxfr (vmr)
...
!            ratecon_sfstrat needs total hcl.
    ...
    call usrrxt( reaction_rates, tfld, ion_temp_fld, ele_temp_fld, invariants, h2ovmr, o2vmr, &
    ...
    if ( xactive_prates ) then
       ...
       if (o2_ndx>0 .and. o3_ndx>0 .and. no2_ndx>0 .and. no_ndx>0) then
          ...
          ! SO2 column.
          if (do_actinic_so2) then  
            if (so2_ndx>0) then
               so24rad(:,:) = vmr(:,:,so2_ndx)
            else
              call endrun('gas_phase_chemdr: must include SO2 for radxfr_do_so2')
            end if
          else
             so24rad(:,:) = 0._r8
          end if
          ...
          if (do_actinic_co2) then
            if (co2_ndx>0) then
              ...
              call endrun('gas_phase_chemdr: must include CO2 for radxfr_do_co2')
            end if
          else
             co24rad(:,:) = 0._r8
          end if
          ...
          if (do_actinic_h2o) then
            if (h2o_ndx>0) then
              ...
              call endrun('gas_phase_chemdr: must include H2O for radxfr_do_h2o')
            end if
          else
             h2o4rad(:,:) = 0._r8
          end if
          ...
          if (do_actinic_hbr) then
            if (hbr_ndx>0) then
              ...
              call endrun('gas_phase_chemdr: must include HBR for radxfr_do_hbr')
            end if
          else
             hbr4rad(:,:) = 0._r8
          end if
          ...
          if (do_actinic_hcl) then
            if (hcl_ndx>0) then
              ...
              call endrun('gas_phase_chemdr: must include HCL for radxfr_do_hcl')
            end if
          else
             hcl4rad(:,:) = 0._r8
          end if
          ...
          call radxfr_cam_update( ncol, lchnk, esfact, sza, asdir, pmid, zmid, tfld, &
               ...
               co24rad(:,:), h2o4rad(:,:), hbr4rad(:,:), hcl4rad(:,:),  &
