...
contains
  ...
  subroutine marbl_diagnostics_init(  &
    ...
    associate(                                         &
      ...
      ! coupe added the following for E*inh of PHYTOPHAEO
!      lname    = 'Surface E*inh of PHYTOPHAEO'
!      sname    = 'ATM_EPHYTO1'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO1, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if
!
!      ! coupe added the following for E*inh of PHYTOPHAEO
!      lname    = 'Surface net E*inh of PHYTOPHAEO'
!      sname    = 'ATM_EPHYTO1_NET'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO1_NET, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if

      ! coupe added the following for E*inh of ANTARCTIC PHYTOPLANKTON from Boucher et al. (1994)
      !lname    = 'Surface E*inh of Antarctic Phytoplankton'
      !sname    = 'ATM_EPHYTO3'
      !units    = 'unitless'
      !vgrid    = 'none'
      !truncate = .false.
      !call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
      !     ind%ATM_EPHYTO3, marbl_status_log)
      !if (marbl_status_log%labort_marbl) then
      !  call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
      !  return
      !end if

 !     lname    = 'Surface E*inh of Prochlorococcus Neale and Thomas 2017'
 !     sname    = 'ATM_EPHYTO4'
 !     units    = 'unitless'
 !     vgrid    = 'none'
 !     truncate = .false.
 !     call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
 !          ind%ATM_EPHYTO4, marbl_status_log)
 !     if (marbl_status_log%labort_marbl) then
 !       call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
 !       return
 !     end if
!
!      lname    = 'Surface net E*inh of Prochlorococcus Neale and Thomas 2017'
!      sname    = 'ATM_EPHYTO4_NET'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO4_NET, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if
!
!      lname    = 'Surface E*inh of Coccolithophores Lorenzo et al. (2019)'
!      sname    = 'ATM_EPHYTO5'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO5, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if
!
!      lname    = 'Surface net E*inh of Coccolithophores Lorenzo et al. (2019)'
!      sname    = 'ATM_EPHYTO5_NET'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO5_NET, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if

      ! coupe added the following for E*inh 
! ATM_EPHYTO1
      lname    = 'Surface E*inh UVA of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVA'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVA, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVB of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVB'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVB, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVC of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVC'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVC, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVA of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVA_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVA_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVB of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVB_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVB_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVC of phaeodactylum (Diatom)'
      sname    = 'ATM_EPHYTO1UVC_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO1UVC_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
...
! ATM_EPHYTO4
      lname    = 'Surface E*inh UVA of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVA'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVA, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVB of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVB'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVB, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVC of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVC'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVC, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVA of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVA_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVA_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVB of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVB_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVB_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface net E*inh UVC of prochlorococcus (small phytoplankton)'
      sname    = 'ATM_EPHYTO4UVC_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO4UVC_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
...
! ATM_EPHYTO5
      lname    = 'Surface E*inh UVA of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVA'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVA, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVB of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVB'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVB, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVA of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVC'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVC, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVA of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVA_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVA_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVB of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVB_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVB_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVC of Coccolithophores (lorenzo)'
      sname    = 'ATM_EPHYTO5UVC_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO5UVC_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
...
! ATM_EPHYTO6
!      lname    = 'Surface E*inh UVA of Coccolithophores (guan)'
!      sname    = 'ATM_EPHYTO6UVA'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO6UVA, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if
!
!      lname    = 'Surface E*inh UVB of Coccolithophores (guan)'
 !     sname    = 'ATM_EPHYTO6UVB'
 !     units    = 'unitless'
 !     vgrid    = 'none'
 !     truncate = .false.
 !     call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
 !          ind%ATM_EPHYTO6UVB, marbl_status_log)
 !     if (marbl_status_log%labort_marbl) then
 !       call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
 !       return
 !     end if

!      lname    = 'Surface E*inh UVC of Coccolithophores (guan)'
!      sname    = 'ATM_EPHYTO6UVC'
!      units    = 'unitless'
!      vgrid    = 'none'
!      truncate = .false.
!      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
!           ind%ATM_EPHYTO6UVC, marbl_status_log)
!      if (marbl_status_log%labort_marbl) then
!        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
!        return
!      end if
...
! ATM_EPHYTO7
      lname    = 'Surface E*inh UVA of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVA'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVA, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVB of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVB'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVB, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if

      lname    = 'Surface E*inh UVC of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVC'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVC, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVA of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVA_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVA_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVB of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVB_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVB_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
      ...
      lname    = 'Surface E*inh UVC of Synechococcus (Neale)'
      sname    = 'ATM_EPHYTO7UVC_NET'
      units    = 'unitless'
      vgrid    = 'none'
      truncate = .false.
      call diags%add_diagnostic(lname, sname, units, vgrid, truncate,     &
           ind%ATM_EPHYTO7UVC_NET, marbl_status_log)
      if (marbl_status_log%labort_marbl) then
        call marbl_logging_add_diagnostics_error(marbl_status_log, sname, subname)
        return
      end if
  ...
  subroutine marbl_diagnostics_interior_tendency_compute( &
    ...
    ! EPHYTO1 is CULLEN ET AL. 1992 DIATOMS
    diags(ind%ATM_EPHYTO1UVA)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVA_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO1UVA_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVA_NET_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO1UVB)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVB_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO1UVB_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVB_NET_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO1UVC)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVC_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO1UVC_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1UVC_NET_col_frac_id)%field_0d(:)

    ! EPHYTO4 is NEALE AND THOMAS 2017 PROCHLOROCOCCUS SMALL PHYTOPLANKTON
    diags(ind%ATM_EPHYTO4UVA)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVA_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO4UVA_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVA_NET_col_frac_id)%field_0d(:) 
    diags(ind%ATM_EPHYTO4UVB)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVB_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO4UVB_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVB_NET_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO4UVC)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVC_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO4UVC_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4UVC_NET_col_frac_id)%field_0d(:)

    ! EPHYTO5 is LORENZO ET AL. 2019 EMILIANIA HUXLEYI COCCOLITHOPHORES
    diags(ind%ATM_EPHYTO5UVA)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVA_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO5UVA_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVA_NET_col_frac_id)%field_0d(:) 
    diags(ind%ATM_EPHYTO5UVB)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVB_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO5UVB_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVB_NET_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO5UVC)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVC_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO5UVC_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5UVC_NET_col_frac_id)%field_0d(:)

    ! EPHYTO6 is GUAN AND GAO 2010 COCCOLITHOPHORES EMILIANIA HUXLEYI 
    !diags(ind%ATM_EPHYTO6UVA)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVA_col_frac_id)%field_0d(:)  
    !diags(ind%ATM_EPHYTO6UVA_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVA_NET_col_frac_id)%field_0d(:) 
    !diags(ind%ATM_EPHYTO6UVB)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVB_col_frac_id)%field_0d(:)  
    !diags(ind%ATM_EPHYTO6UVB_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVB_NET_col_frac_id)%field_0d(:)
    !diags(ind%ATM_EPHYTO6UVC)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVC_col_frac_id)%field_0d(:)  
    !diags(ind%ATM_EPHYTO6UVC_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO6UVC_NET_col_frac_id)%field_0d(:)

    ! EPHYTO7 is NEALE AND THOMAS 2014 SYNECHOCOCCUS SMALL PHYTOPLANKTON
    diags(ind%ATM_EPHYTO7UVA)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVA_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO7UVA_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVA_NET_col_frac_id)%field_0d(:) 
    diags(ind%ATM_EPHYTO7UVB)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVB_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO7UVB_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVB_NET_col_frac_id)%field_0d(:)
    diags(ind%ATM_EPHYTO7UVC)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVC_col_frac_id)%field_0d(:)  
    diags(ind%ATM_EPHYTO7UVC_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO7UVC_NET_col_frac_id)%field_0d(:)

        
    ! add ATM_EPHYTO1 from interior_tendency_forcings to MARBL diagnostics for output 
    !diags(ind%ATM_EPHYTO1)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1_col_frac_id)%field_0d(:)  !ephyto1
    !diags(ind%ATM_EPHYTO1_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO1_NET_col_frac_id)%field_0d(:)  !ephyto1
    !diags(ind%ATM_EPHYTO3)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO3_col_frac_id)%field_0d(:) !ephyto3
    !diags(ind%ATM_EPHYTO4)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4_col_frac_id)%field_0d(:)  !ephyto4
    !diags(ind%ATM_EPHYTO5)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5_col_frac_id)%field_0d(:)  !ephyto5
    !diags(ind%ATM_EPHYTO4_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO4_NET_col_frac_id)%field_0d(:)  !ephyto4
    !diags(ind%ATM_EPHYTO5_NET)%field_2d =interior_tendency_forcings(interior_tendency_forcing_ind%EPHYTO5_NET_col_frac_id)%field_0d(:)  !ephyto5
    end associate
