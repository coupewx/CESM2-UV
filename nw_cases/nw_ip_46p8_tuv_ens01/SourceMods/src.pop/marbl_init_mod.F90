...
! EPHYTO1 : CULLEN ET AL. 2019 PHAEODACTYLUM (DIATOMS) USING E MODEL
        if (id .eq. ind%EPHYTO1UVA_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto1UVA'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO1UVB_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto1UVB'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO1UVC_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto1UVC'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO1UVA_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto1UVA'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO1UVB_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto1UVB'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO1UVC_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto1UVC'! - phaoedactylum from Cullen et al. (1992)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

! EPHYTO4 : NEALE AND THOMAS 2017 PROCHLOROCOCCUS USING EMAX MODEL

        if (id .eq. ind%EPHYTO4UVA_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto4UVA'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO4UVB_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto4UVB'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO4UVC_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto4UVC'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO4UVA_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto4UVA'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO4UVB_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto4UVB'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO4UVC_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto4UVC'! - prochlorococcus from Neale and Thomas (2017)'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

! EPHYTO5 : LORENZO ET AL 2019 COCCOLITHOPHORES USING T MODEL (ABANDON IN THE FUTURE)

        if (id .eq. ind%EPHYTO5UVA_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto5UVA'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO5UVB_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto5UVB'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO5UVC_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto5UVC'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO5UVA_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto5UVA'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO5UVB_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto5UVB'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO5UVC_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto5UVC'! - coccolithophores from Lorenzo et al. 2019'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

! EPHYTO6 : GUAN AND GAO (2010) COCCOLITHIPHORES USING E MODEL

! EPHYTO7 : NEALE AND THOMAS (2017) - SYNECHOCOCCUS

        if (id .eq. ind%EPHYTO7UVA_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto7UVA'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO7UVB_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto7UVB'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO7UVC_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Inhibition Phyto7UVC'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO7UVA_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto7UVA'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO7UVB_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto7UVB'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

        if (id .eq. ind%EPHYTO7UVC_NET_col_frac_id) then
          found = .true.
          interior_tendency_forcings(id)%metadata%varname     = 'Net Inhibition Phyto7UVC'! - synechococcus from NEALE AND THOMAS 2017'
          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        end if

!
 !       if (id .eq. ind%EPHYTO1_col_frac_id) then
 !         found = .true.
 !         interior_tendency_forcings(id)%metadata%varname     = 'UV Inhibition Phyto1'! - phaoedactylum from Cullen et al. (1992)'
!          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
!          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
!        end if
!
 !       if (id .eq. ind%EPHYTO1_NET_col_frac_id) then
 !         found = .true.
 !         interior_tendency_forcings(id)%metadata%varname     = 'Net UV Inhibition Phyto1'! - phaoedactylum from Cullen et al. (1992)'
 !         interior_tendency_forcings(id)%metadata%field_units = 'unitless'
 !         call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
 !       end if        

        !if (id .eq. ind%EPHYTO3_col_frac_id) then !ephyto3
        !  found = .true.
        !  interior_tendency_forcings(id)%metadata%varname     = 'UV Inhibition Phyto3'! - antarctic phyto from Boucher et al. (1994)'
        !  interior_tendency_forcings(id)%metadata%field_units = 'unitless'
        !  call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
        !end if
!
!        if (id .eq. ind%EPHYTO4_col_frac_id) then
!          found = .true.
!          interior_tendency_forcings(id)%metadata%varname     = 'UV Inhibition Phyto4'! - Neale and Thomas (2017) procochclcorucouso'
!          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
!          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
!        end if
!
!        if (id .eq. ind%EPHYTO4_NET_col_frac_id) then
!          found = .true.
!          interior_tendency_forcings(id)%metadata%varname     = 'Net UV Inhibition Phyto4'! - Neale and Thomas (2017) procochclcorucouso'
!          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
!          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
!        end if
!
!        if (id .eq. ind%EPHYTO5_col_frac_id) then
!          found = .true.
!          interior_tendency_forcings(id)%metadata%varname     = 'UV Inhibition Phyto5'! - Lorenzo et al. (2019) -coccolithophore'
!          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
!          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
!        end if
!
!        if (id .eq. ind%EPHYTO5_NET_col_frac_id) then
!          found = .true.
!          interior_tendency_forcings(id)%metadata%varname     = 'Net UV Inhibition Phyto5'! - Lorenzo et al. (2019) -coccolithophore'
!          interior_tendency_forcings(id)%metadata%field_units = 'unitless'
!          call interior_tendency_forcings(id)%set_rank(num_elements, 0, marbl_status_log)!, dim1 = num_PAR_subcols)
!        end if
!
