...
subroutine microfast(carma, cstate, iz, scale_threshold, rc)
  ...
  if (do_grow) then
    ...
    if ((igash2o /= 0) .and. (igash2so4 /= 0)) then
      ...
      if ((gc(iz, igash2o) > 1.0e-23_f) .and. (gc(iz,igash2so4) > 1.0e-23_f)) then
  ...
  ! NOTE: Oxidation can also result in evaporation of particle,
  ! so for now do_grow needs to be enabled for the evaporation. This
  ! could be unwound in the future.
  if (do_oxid) then
    call oxidize(carma, cstate, iz, rc)
    if (rc < RC_OK) return
  end if  
...
!          if (srat >= ds_threshold(igas)) then
      ...
      ! evaporation and a potential overshoot in the result.
        ...
        if (nretries >= (0.95_f * maxretries)) then
          s_threshold = 16._f  * s_threshold
        else if (nretries >= (0.9_f * maxretries)) then
          s_threshold = 8._f  * s_threshold
        else if (nretries >= (0.8_f * maxretries)) then
        ...
        if (((supsatnew * supsatold) < 0._f) .and. (abs(supsatnew) > s_threshold)) then
          ...
          if (do_substep) then
            if (nretries == maxretries) then 
              ...
              gc(iz,igas)   = 0.0_f
              supsatnew = 0.0_f
              supsati(iz,igas) = 0.0_f
              supsatl(iz,igas) = 0.0_f
!              t(iz) = T0
            end if
          else
            if (do_print) write(LUNOPRT,3) trim(gasname(igas)), iz, &
