...
contains
  ...
  subroutine CARMA_Create(carma, NBIN, NELEM, NGROUP, NSOLUTE, NGAS, NWAVE, rc, &
    ...
    allocate( &
      ...
      carma%f_noxd2elem(NELEM), &
      carma%f_ioxd2elem(NELEM), &
      stat=ier) 
    if(ier /= 0) then
    ...
    if (NGAS > 0) then
      allocate( &
        carma%f_gas(NGAS), &
        carma%f_ioxd2gas(NGAS,NELEM), &
        carma%f_ioxd2gamma(NGAS,NELEM), &
        carma%f_ioxd2wtmol_rxn(NGAS,NELEM), &
        carma%f_is_oxid_gas(NGAS), &
        stat=ier) 
      if(ier /= 0) then
      ...
      carma%f_ioxd2gas(:,:) = 0
      carma%f_ioxd2gamma(:,:) = 0._f
      carma%f_ioxd2wtmol_rxn(:,:) = 0._f
      carma%f_is_oxid_gas(:) = .false.
    end if
  ...
  subroutine CARMA_Initialize(carma, rc, do_cnst_rlh, do_coag, do_detrain, do_fixedinit, &
      do_grow, do_grow_limit, do_incloud, do_explised, do_print_init, do_substep, do_thermo, do_vdiff, &
      do_vtran, do_drydep, vf_const, minsubsteps, maxsubsteps, maxretries, glim_retries, glim_scale, conmax, &
      do_pheat, do_pheatatm, dt_threshold, cstick, gsticki, gstickl, tstick, do_clearsky, &
      do_partialinit, do_oxid)
    ...
    logical, intent(in), optional       :: do_grow_limit !! do growth limits?
    logical, intent(in), optional       :: do_oxid       !! do oxidation?
    ...
    integer, intent(in), optional       :: glim_retries  !! number of substep retries for growth limit, default = 5
    real(kind=f), intent(in), optional  :: glim_scale    !! minimum relative concentration to consider, default = 1e-1
    ...
    carma%f_do_grow_limit = .FALSE.
    carma%f_do_oxid       = .FALSE.
    ...
    if (present(do_grow_limit)) carma%f_do_grow_limit = do_grow_limit
    if (present(do_oxid))       carma%f_do_oxid       = do_oxid
    ...
    carma%f_glim_retries = 1        ! growth limit retries
    carma%f_glim_scale  = 0.9_f     ! growth limit scale
    ...
    if (present(glim_retries))  carma%f_glim_retries  = glim_retries
    if (present(glim_scale))  carma%f_glim_scale  = glim_scale
      ...
      do igroup = 1, carma%f_NGROUP
        ...
        if (carma%f_group(igroup)%f_do_mie) then 
          ...
          do ibin = 1, carma%f_NBIN
            ...
            rcore = ((carma%f_group(igroup)%f_fraccore * carma%f_group(igroup)%f_rhopart / &
                     carma%f_group(igroup)%f_rhocore) ** (1./3.)) * carma%f_group(igroup)%f_r(ibin) 
...
!    carma%f_if_nuc(ielemfrom,carma%f_inuc2elem(carma%f_nnuc2elem(ielemfrom), ielemfrom)) = .true.
  ...
  !! Add an oxidation process that has a gas (<i>gas</i>) oxidize the volatile component of a 
  !! particle (<i>elemfrom</i>). The elements and gas should have already been defined
  !! using <i>CARMA_AddElement()</i> and <i>CARMA_AddGas()</i>.
  !! 
  !! Total oxidation transfers particle mass from the destination element back to the
  !! element indicated by <i>ioxd2elem</i>.
  !!
  !! @author Chuck Bardeen
  !! @version Jun-2019
  !! @see CARMA_AddElement
  !! @see CARMA_AddGas
  subroutine CARMA_AddOxidation(carma, ielemfrom, igas, gamma, wtmol_rxn, rc, ievp2elem)
    ...
    integer, intent(in)                :: ielemfrom   !! the source element
    integer, optional, intent(in)      :: igas        !! the gas
    real(kind=f), intent(in)           :: gamma       !! reaction probability
    real(kind=f), intent(in)           :: wtmol_rxn   !! molecular weight of aerosol lost per reaction with gas
    integer, intent(out)               :: rc          !! return code, negative indicated failure
    integer, optional, intent(in)      :: ievp2elem   !! the element created upon total oxidation
    ...
    ! Make sure the elements exist.
    if (ielemfrom > carma%f_NELEM) then
      if (carma%f_do_print) write(carma%f_LUNOPRT, *) "CARMA_AddOxidation:: ERROR - The specifed element (", &
        ielemfrom, ") is larger than the number of elements (", carma%f_NELEM, ")."
      rc = RC_ERROR
      return
    end if
    ...
    if (present(igas)) then
      if (igas > carma%f_NGAS) then
        if (carma%f_do_print) write(carma%f_LUNOPRT, *) "CARMA_AddOxidation:: ERROR - The specifed gas (", &
          igas, ") is larger than the number of gases (", carma%f_NGAS, ")."
        rc = RC_ERROR
        return
      end if
    ...
    ! Oxidation transfers particle mass from element <ielem> to <igas>
    ! where <i> ranges from 0 to the number of gases oxidizing <ielem>.
    carma%f_noxd2elem(ielemfrom) = carma%f_noxd2elem(ielemfrom) + 1
    carma%f_ioxd2gas(carma%f_noxd2elem(ielemfrom), ielemfrom) = igas
    carma%f_ioxd2gamma(carma%f_noxd2elem(ielemfrom), ielemfrom) = gamma
    carma%f_ioxd2wtmol_rxn(carma%f_noxd2elem(ielemfrom), ielemfrom) = wtmol_rxn
    carma%f_is_oxid_gas(igas) = .true.

    ! Total evaporation mapping: total evaporation transfers particle mass from
    ! element <ielem> to element <ievp2elem(ielem)>.
    if (present(ievp2elem)) carma%f_ievp2elem(ielemfrom) = ievp2elem
  ...
  subroutine CARMA_Get(carma, rc, LUNOPRT, NBIN, NELEM, NGAS, NGROUP, NSOLUTE, NWAVE, do_detrain, &
    do_drydep, do_fixedinit, do_grow, do_oxid, do_print, do_print_init, do_thermo, wave, dwave, do_wave_emit, &
    do_partialinit)
    ...
    logical, optional, intent(out)      :: do_oxid              !! do oxidation
    ...
    if (present(do_oxid))       do_oxid        = carma%f_do_oxid
