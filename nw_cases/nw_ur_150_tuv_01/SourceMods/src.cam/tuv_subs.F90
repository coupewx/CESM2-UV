      ...
      SUBROUTINE tuv_radfld( nlambda_start, cld_od_opt, cldfrac, nlev, nwave, &
                             zenith, z, albedo, &
                             aircol, o3col, so2col, no2col, nocol, &
                             co2col, h2ocol, hbrcol, hclcol, &
                             dtaer, omaer, gaer, dtcld, omcld, gcld, &
                             qll, dobsi, o3_xs, no2_xs, no_xs, o2_xs, &
                             so2_xs, co2_xs, h2o_xs, hbr_xs, hcl_xs, &
                             wmin, wc, tlev, dto2, radfld, efld, &
                             e_dir, e_dn, e_up, &
...
!-----------------------------------------------------------------------------
      ...
      real(rk), intent(in)  :: nocol(:)
      real(rk), intent(in)  :: co2col(:)
      real(rk), intent(in)  :: h2ocol(:)
      real(rk), intent(in)  :: hbrcol(:)
      real(rk), intent(in)  :: hclcol(:)
      ...
      real(rk), intent(in)  :: no_xs(:)
      real(rk), intent(in)  :: co2_xs(:)
      real(rk), intent(in)  :: h2o_xs(:)
      real(rk), intent(in)  :: hbr_xs(:)
      real(rk), intent(in)  :: hcl_xs(:)
...
!-----------------------------------------------------------------------------
      ...
      real(rk) :: dtno(nlev,nwave)
      real(rk) :: dtco2(nlev,nwave)
      real(rk) :: dth2o(nlev,nwave)
      real(rk) :: dthbr(nlev,nwave)
      real(rk) :: dthcl(nlev,nwave)
...
!        dtaer(:,wn) = 0._rk
      ...
      call setno( nocol, no_xs, dtno )
      call setco2( co2col, co2_xs, dtco2 )
      call seth2o( h2ocol, h2o_xs, dth2o )
      call sethbr( hbrcol, hbr_xs, dthbr )
      call sethcl( hclcol, hcl_xs, dthcl )
...
!-----------------------------------------------------------------------------
      ...
      SUBROUTINE setno( colno, no_xs, dtno )
!-----------------------------------------------------------------------------
!=  NAME:  Optical Depths of no2
!=  PURPOSE:
!=  Compute no2 optical depths as a function of altitude and wavelength
...
!     ... dummy arguments
!-----------------------------------------------------------------------------
      REAL(rk), intent(in)    :: colno(:)
      REAL(rk), intent(in)    :: no_xs(:)
      REAL(rk), intent(inout) :: dtno(:,:)
      ...
      DO wn = 1,nwave
        dtno(:nlyr,wn) = colno(:nlyr)*no_xs(wn)
      END DO

      END SUBROUTINE setno

      SUBROUTINE setco2( colco2, co2_xs, dtco2 )
!-----------------------------------------------------------------------------
!=  NAME:  Optical Depths of co2
!=  PURPOSE:
!=  Compute co2 optical depths as a function of altitude and wavelength
!-----------------------------------------------------------------------------
!=  PARAMETERS:
!=  CO2_XS - REAL, molecular absorption cross section (cm^2) of co2 at    (I)
!=           each specified wavelength and altitude
!=  COLCO2 - REAL, co2 vertical column increments, molec cm-2, for each   (I)
!=           layer
!=  DTCO2  - REAL, optical depth due to no2 absorption at each            (O)
!=           specified altitude at each specified wavelength
!-----------------------------------------------------------------------------
...
!     ... dummy arguments
!-----------------------------------------------------------------------------
      REAL(rk), intent(in)    :: colco2(:)
      REAL(rk), intent(in)    :: co2_xs(:)
      REAL(rk), intent(inout) :: dtco2(:,:)
      ...
      DO wn = 1,nwave
        dtco2(:nlyr,wn) = colco2(:nlyr)*co2_xs(wn)
      END DO

      END SUBROUTINE setco2

      SUBROUTINE seth2o( colh2o, h2o_xs, dth2o )
!-----------------------------------------------------------------------------
!=  NAME:  Optical Depths of h2o
!=  PURPOSE:
!=  Compute h2o optical depths as a function of altitude and wavelength
!-----------------------------------------------------------------------------
!=  PARAMETERS:
!=  H2O_XS - REAL, molecular absorption cross section (cm^2) of h2o at    (I)
!=           each specified wavelength and altitude
!=  COLH2O - REAL, h2o vertical column increments, molec cm-2, for each   (I)
!=           layer
!=  DTH2O  - REAL, optical depth due to h2o absorption at each            (O)
!=           specified altitude at each specified wavelength
!-----------------------------------------------------------------------------
...
!     ... dummy arguments
!-----------------------------------------------------------------------------
      REAL(rk), intent(in)    :: colh2o(:)
      REAL(rk), intent(in)    :: h2o_xs(:)
      REAL(rk), intent(inout) :: dth2o(:,:)
      ...
      DO wn = 1,nwave
        dth2o(:nlyr,wn) = colh2o(:nlyr)*h2o_xs(wn)
      END DO

      END SUBROUTINE seth2o

      SUBROUTINE sethbr( colhbr, hbr_xs, dthbr )
!-----------------------------------------------------------------------------
!=  NAME:  Optical Depths of hbr
!=  PURPOSE:
!=  Compute hbr optical depths as a function of altitude and wavelength
!-----------------------------------------------------------------------------
!=  PARAMETERS:
!=  HBR_XS - REAL, molecular absorption cross section (cm^2) of hbr at    (I)
!=           each specified wavelength and altitude
!=  COLHBR - REAL, hbr vertical column increments, molec cm-2, for each   (I)
!=           layer
!=  DTHBR  - REAL, optical depth due to hbr absorption at each            (O)
!=           specified altitude at each specified wavelength
!-----------------------------------------------------------------------------
...
!     ... dummy arguments
!-----------------------------------------------------------------------------
      REAL(rk), intent(in)    :: colhbr(:)
      REAL(rk), intent(in)    :: hbr_xs(:)
      REAL(rk), intent(inout) :: dthbr(:,:)
      ...
      DO wn = 1,nwave
        dthbr(:nlyr,wn) = colhbr(:nlyr)*hbr_xs(wn)
      END DO

      END SUBROUTINE sethbr

      SUBROUTINE sethcl( colhcl, hcl_xs, dthcl )
!-----------------------------------------------------------------------------
!=  NAME:  Optical Depths of hcl
!=  PURPOSE:
!=  Compute hcl optical depths as a function of altitude and wavelength
!-----------------------------------------------------------------------------
!=  PARAMETERS:
!=  HCL_XS - REAL, molecular absorption cross section (cm^2) of hcl at    (I)
!=           each specified wavelength and altitude
!=  COLHCL - REAL, hcl vertical column increments, molec cm-2, for each   (I)
!=           layer
!=  DTHCL  - REAL, optical depth due to hcl absorption at each            (O)
!=           specified altitude at each specified wavelength
!-----------------------------------------------------------------------------
...
!     ... dummy arguments
!-----------------------------------------------------------------------------
      REAL(rk), intent(in)    :: colhcl(:)
      REAL(rk), intent(in)    :: hcl_xs(:)
      REAL(rk), intent(inout) :: dthcl(:,:)
      ...
      DO wn = 1,nwave
        dthcl(:nlyr,wn) = colhcl(:nlyr)*hcl_xs(wn)
      END DO

      END SUBROUTINE sethcl
