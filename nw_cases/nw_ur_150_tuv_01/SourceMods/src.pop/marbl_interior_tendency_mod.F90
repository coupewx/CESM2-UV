module marbl_interior_tendency_mod
  ...
  use forcing_fields, only : ATM_PAR     ! COUPE ADDED 2/15/2023
  !use forcing_fields, only : ATM_EPHYTO1
  !use forcing_fields, only : ATM_EPHYTO2
  !use forcing_fields, only : ATM_EPHYTO3
  !use forcing_fields, only : ATM_EPHYTO4
  !use forcing_fields, only : ATM_EPHYTO5
...
contains
  ...
  subroutine marbl_interior_tendency_compute( &
    ...
    associate(                                                      &
         ...
         ! the below lines are where e*inh are brought in from atm to cpl to ocn - changed to net uv inh to account for ice albedo
         !atm_ephyto1         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO1_NET_col_frac_id)%field_0d(1),&   ! COUPE added
         !atm_ephyto3         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO3_col_frac_id)%field_0d(1),&   ! COUPE added
         !atm_ephyto4         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO4_NET_col_frac_id)%field_0d(1),&   ! COUPE added
         !atm_ephyto5         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO5_NET_col_frac_id)%field_0d(1),&   ! COUPE added
         atm_ephyto1UVA         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO1UVA_NET_col_frac_id)%field_0d(1),&
         atm_ephyto1UVB         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO1UVB_NET_col_frac_id)%field_0d(1),&
         atm_ephyto1UVC         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO1UVC_NET_col_frac_id)%field_0d(1),&

         atm_ephyto4UVA         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO4UVA_NET_col_frac_id)%field_0d(1),&
         atm_ephyto4UVB         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO4UVB_NET_col_frac_id)%field_0d(1),&
         atm_ephyto4UVC         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO4UVC_NET_col_frac_id)%field_0d(1),&

         atm_ephyto5UVA         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO5UVA_NET_col_frac_id)%field_0d(1),&
         atm_ephyto5UVB         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO5UVB_NET_col_frac_id)%field_0d(1),&
         atm_ephyto5UVC         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO5UVC_NET_col_frac_id)%field_0d(1),&

         atm_ephyto7UVA         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO7UVA_NET_col_frac_id)%field_0d(1),&
         atm_ephyto7UVB         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO7UVB_NET_col_frac_id)%field_0d(1),&
         atm_ephyto7UVC         =>interior_tendency_forcings(interior_tendency_forcing_indices%EPHYTO7UVC_NET_col_frac_id)%field_0d(1),&
    ...
    !     Tfunc_auto(:,:), PAR%col_frac(:), PAR%avg(:,:),autotroph_derived_terms,atm_ephyto1,atm_ephyto4,atm_ephyto5,totalChl_local,domain) ! Coupe added atm_ephyto1 and 3

   ! call compute_autotroph_photosynthesis_UVinh(km,num_PAR_subcols,autotroph_local,temperature(:),Tfunc_auto(:,:), PAR%col_frac(:), PAR%avg(:,:),autotroph_derived_terms, &
   !      atm_ephyto1,atm_ephyto4,atm_ephyto5,totalChl_local,domain)

    call compute_autotroph_photosynthesis_UVABCinh(km,num_PAR_subcols,autotroph_local,temperature(:),Tfunc_auto(:,:), PAR%col_frac(:), PAR%avg(:,:),autotroph_derived_terms, &
         atm_ephyto1UVA,atm_ephyto1UVB,atm_ephyto1UVC,atm_ephyto4UVA,atm_ephyto4UVB,atm_ephyto4UVC,atm_ephyto5UVA,atm_ephyto5UVB,atm_ephyto5UVC, &
         atm_ephyto7UVA,atm_ephyto7UVB,atm_ephyto7UVC,totalChl_local,domain)
  ...
  subroutine compute_autotroph_photosynthesis(km, PAR_nsubcols, &
       ...
       autotroph_derived_terms,ATM_EPHYTO1,ATM_EPHYTO4,ATM_EPHYTO5,CHL,domain) ! coupe added atm_Ephyto1, CHL, domain
    ...
    type(autotroph_derived_terms_type), intent(inout) :: autotroph_derived_terms
    real(r8),                           intent(in)   :: ATM_EPHYTO1 !  Cullen et al. (1992) BWF for diatoms
    real(r8),                           intent(in)   :: ATM_EPHYTO4 !  NEale and Thomas (2017)
    real(r8),                           intent(in)   :: ATM_EPHYTO5 !  Lorenzo et al. (2019)
    real(r8),                           intent(in)   :: CHL(:) ! chlorophyll ( mg / m3)
    type(marbl_domain_type),            intent(in)   :: domain
    ...
    real(r8) :: work             ! intermediate term common to multiple expressionsi
    real(r8) :: uv_limitation    ! uv limitation term
    real(r8) :: E_inh ! in loop ephyto1 or ephyto3
    real(r8) :: gamma_scale ! factor to scale E*inh by depending on PICPOC
    real(r8),parameter :: PAR_threshold=1.0e-19_r8  ! below this threshold small enough to have no effect
    real(r8) :: WORK1(domain%kmt)
    real(r8) :: KPARdz    !K * dz - an attenutation coefficient integrated with respect to dz
    !-----------------------------------------------------------------------
    ...
    associate(                                                &
         ...
         PICPOC    => autotroph_derived_terms%QCaCO3(:,:),     & ! current CaCO3/C ratio (mmol CaCO3/mmol C)
         delta_z   => domain%delta_z,                          &
         column_kmt=>domain%kmt                        &
         )

      gamma_scale=(1.0_r8/3.5_r8) ! based on shelled coccos having 3.5 growth rate as naked coccolithophores
      WORK1(:) = max(CHL(1:column_kmt),0.02_r8) ! so chlorophyll can be used in column attenuation of UV - min value in CESM for vis light is 0.02 
      do auto_ind = 1, autotroph_cnt ! cycle through phytoplankton functional types
        if (auto_ind==1) then ! Small Phytoplankton
         E_inh=ATM_EPHYTO4  ! use Neale and Thomas (2017) for prochlorococcus aka small phytoplankton
        else if (auto_ind==2) then ! Diatoms
         E_inh=ATM_EPHYTO1 ! Cullen et al. (1992) - Diatoms
        else if (auto_ind==3) then ! Diazotrophs
         E_inh=ATM_EPHYTO4  ! use Neale and Thomas (2017) because diazotrophs are most like small phytoplankton
        else if (auto_ind==4) then ! coccolithophores
         E_inh=ATM_EPHYTO5 ! use Lorenzo et al. (2019) - emiliana huxleyi
        end if
        !E_inh = 0.0_r8!ATM_EPHYTO1  ! set this if want to hard-code E*inh to be from Cullen et al. (1992) - this was done prior 
        do k=1,km ! cycle through ocean layers
          if (temperature(k) < autotroph_settings(auto_ind)%temp_thres) then ! if temperature below a critical threshold, photosynthesis_max=0
            PCmax = c0
          else
            PCmax = autotroph_settings(auto_ind)%PCref * f_nut(auto_ind,k) * Tfunc_auto(auto_ind,k) !otherwise it's a function of nutrients and temperature
          end if
          if (thetaC(auto_ind,k) > c0) then
            light_lim(auto_ind,k) = c0
            PCphoto(auto_ind,k)   = c0
            photoacc(auto_ind,k)  = c0
            uv_limitation=c1 ! by default uv limitation is set to 1 - no limitation
            if (k<20) then ! hard-code in that it is only the top 20 layers (~200 m) that are affected by E*inh
                 KPARdz = 0.14*WORK1(k) + 0.29  ! linear reg of CHL onto UV penetration from Overmans and Agusti (2020) for Red Sea
                 KPARdz = KPARdz * delta_z(k)*0.01 ! note MARBL units for delta_z are CENTIMETERS and lin reg is for METERS (annoying)
                 !WRITE(*,*) delta_z(k) ! debugging attempt
                 E_inh = E_inh * exp(-KPARdz) ! propagation of E*inh thru column
                 if (E_inh < PAR_threshold) then ! to save ourselves computing resources abort if E_inh is extremely small 
                   E_inh =c0
                 end if
                 !WRITE(*,*) E_inh ! debugging attempt
                 uv_limitation = 1 / (1+E_inh) ! for all PFTs, calculate gamma = 1 / (1+E*inh)
                 if (auto_ind==4) then  ! if coccolithophores, add enhancement if PICPOC is low - level specific !
                   !uv_limitation = 1 / (1+E_inh)
                   if (E_inh>0.001) then
                    if (PICPOC(auto_ind,k) .LE. 0.1) then ! if PICPOC<0.1, defining it as 'naked' and scaling uv_limitation by 1/3.5.
                      uv_limitation = uv_limitation*gamma_scale ! based on 3.5x growth rate diff between naked vs shelled cocco in Xu et al. (2016)
                    end if
                    if (PICPOC(auto_ind,k) .GT. 0.1) then !linear slope from 1.18 to 1.0  between PICPOC=0.1 and PICPOC=1.0
                      !gamma_scale=-1*(1.18-1.0)*(0.9)*(PICPOC(auto_ind,k)-0.1) + 1.18_r8
                      uv_limitation = uv_limitation * abs(PICPOC(auto_ind,k))**(1.0/6.0)! ( ((gamma_scale)- 1)/0.9 *-1)*(PICPOC(auto_ind,k)-0.1)+(gamma_scale)  !
                    endif
                   end if
                 end if
            end if
            ...
            do subcol_ind = 1, PAR_nsubcols
              ...
              if (work > c0) then
                ...
                PCphoto_subcol = PCmax * light_lim_subcol*uv_limitation ! using the same uv_limitation for every subcol seems like the right move
  ...
  subroutine compute_autotroph_photosynthesis_UVABCinh(km, PAR_nsubcols, &
       autotroph_local, temperature, Tfunc_auto, PAR_col_frac, PAR_avg, &
       autotroph_derived_terms,ATM_EPHYTO1UVA,ATM_EPHYTO1UVB,ATM_EPHYTO1UVC,&
       ATM_EPHYTO4UVA,ATM_EPHYTO4UVB,ATM_EPHYTO4UVC,&
       ATM_EPHYTO5UVA,ATM_EPHYTO5UVB,ATM_EPHYTO5UVC,&
       ATM_EPHYTO7UVA,ATM_EPHYTO7UVB,ATM_EPHYTO7UVC,&
       CHL,domain) ! coupe added atm_Ephyto1, CHL, domain
    ! JLC 8/29 CREATED NEW SUBROUTINE THAT INCLUDES UV_INH
    !-----------------------------------------------------------------------
    !     get photosynth. rate, phyto C biomass change, photoadapt
    !-----------------------------------------------------------------------
    ...
    !real(r8),                           intent(in)   :: ATM_EPHYTO4 !  NEale and Thomas (2017)
    !real(r8),                           intent(in)   :: ATM_EPHYTO5 !  Lorenzo et al. (2019)
    real(r8),                           intent(in)   :: ATM_EPHYTO1UVA
    real(r8),                           intent(in)   :: ATM_EPHYTO1UVB
    real(r8),                           intent(in)   :: ATM_EPHYTO1UVC

    real(r8),                           intent(in)   :: ATM_EPHYTO4UVA
    real(r8),                           intent(in)   :: ATM_EPHYTO4UVB
    real(r8),                           intent(in)   :: ATM_EPHYTO4UVC

    real(r8),                           intent(in)   :: ATM_EPHYTO5UVA
    real(r8),                           intent(in)   :: ATM_EPHYTO5UVB
    real(r8),                           intent(in)   :: ATM_EPHYTO5UVC

    real(r8),                           intent(in)   :: ATM_EPHYTO7UVA
    real(r8),                           intent(in)   :: ATM_EPHYTO7UVB
    real(r8),                           intent(in)   :: ATM_EPHYTO7UVC
    real(r8),                           intent(in)   :: CHL(:) ! chlorophyll ( mg / m3)
    type(marbl_domain_type),            intent(in)   :: domain
    ...
    real(r8) :: E_inhUVC
    real(r8) :: E_inh
    LOGICAL :: T_model, E_model, Emax_model
    real(r8) :: Emax_value
    real(r8) :: CVALUE
    real(r8) :: gamma_scale ! factor to scale E*inh by depending on PICPOC
    real(r8),parameter :: PAR_threshold=1.0e-19_r8  ! below this threshold small enough to have no effect
    ...
    associate(                                                &
      ...
      !E_model=.FALSE.
      !Emax_model=.FALSE.
      gamma_scale=(1.0_r8/1.75_r8) ! based on shelled coccos having 3.5x growth rate as naked coccolithophores
      WORK1(:) = max(CHL(1:column_kmt),0.02_r8) ! so chlorophyll can be used in column attenuation of UV - min value in CESM for vis light is 0.02 
      do auto_ind = 1, autotroph_cnt ! cycle through phytoplankton functional types
       T_model=.FALSE.
       E_model=.FALSE.
       Emax_model=.FALSE.
       if (auto_ind==1 .or. auto_ind==3) then ! Small Phytoplankton or diazotrophs
         !E_inhUVA=ATM_EPHYTO4UVA  ! use Neale and Thomas (2017) for prochlorococcus aka small phytoplankton
         !E_inhUVB=ATM_EPHYTO4UVB
         !E_inhUVC=ATM_EPHYTO4UVC
         !Emax_model=.TRUE.
         !Emax_value = 0.85 ! ML 20C E*max is 0.85 +/- 0.12
         !CVALUE = (1+Emax_value) / (Emax_value)
         !Synechococcus Neale and Thomas (2014) - also for small phytoplankton
         E_inhUVA=ATM_EPHYTO7UVA
         E_inhUVB=ATM_EPHYTO7UVB
         E_inhUVC=ATM_EPHYTO7UVC
         Emax_model=.TRUE.
         Emax_value = 0.54 !ML20C for WH7803 is 0.54
         CVALUE = (1+Emax_value) / (Emax_value)
        else if (auto_ind==2) then ! Diatoms
         E_inhUVA=ATM_EPHYTO1UVA ! Cullen et al. (1992) - Diatoms
         E_inhUVB=ATM_EPHYTO1UVB
         E_inhUVC=ATM_EPHYTO1UVC
         E_model=.TRUE.
        !else if (auto_ind==3) then ! Diazotrophs
        ! E_inhUVA=ATM_EPHYTO4UVA  ! use Neale and Thomas (2017) because diazotrophs are most like small phytoplankton
        ! E_inhUVB=ATM_EPHYTO4UVB
        ! E_inhUVC=ATM_EPHYTO4UVC
        ! Emax_model=.TRUE.
        ! Emax_value=0.85 !ML 20C E*max is 0.85 +/- 0.12
        !CVALUE = (1+Emax_value) / (Emax_value)
         ...
         E_inhUVC=ATM_EPHYTO5UVC ! use Lorenzo et al. (2019) - emiliana huxleyi
         E_inhUVA=ATM_EPHYTO5UVA
         E_inhUVB=ATM_EPHYTO5UVB
         T_model = .TRUE.
        ! E_inhUVA=abs(ATM_EPHYTO6UVA) ! using Guan and Gao (2010) for emiliania huxleyi
        ! E_inhUVB=abs(ATM_EPHYTO6UVB)
        ! E_inhUVC=abs(ATM_EPHYTO6UVC)
        ! E_model=.TRUE.

        end if
        !E_inh = E_inhUVA + E_inhUVB + E_inhUVC ! this line is only valid with OLD propagation patterns !
        !E_inh = 0.0_r8!ATM_EPHYTO1  ! set this if want to hard-code E*inh to be from Cullen et al. (1992) - this was done prior 
        do k=1,km ! cycle through ocean layers
          if (temperature(k) < autotroph_settings(auto_ind)%temp_thres) then ! if temperature below a critical threshold, photosynthesis_max=0
            PCmax = c0
          ...
          if (thetaC(auto_ind,k) > c0) then
            ...
            if (k<20) then ! hard-code in that it is only the top 20 layers (~200 m) that are affected by E*inh
                 ...
                 ! UV-A RADIATION APPROXIMATE ATTENUATION WITH DEPTH
                 !KPARdz = 0.07*WORK1(k) + 0.15 ! Kd(340 nm) = 0.07*CHL + 0.15
                 KPARdz = 0.243728*WORK1(k) + 0.041839 ! Kd(340 nm) = 0.243728*CHL + 0.041839 ! new based on Tedetti et al. (2007) 
                 KPARdz = KPARdz*delta_z(k)*0.01
                 E_inhUVA = E_inhUVA*exp(-KPARdz)

                 !KPARdz = 0.14*WORK1(k) + 0.29  ! linear reg of CHL onto UV penetration from Overmans and Agusti (2020) for Red Sea
                 KPARdz = 0.437891*WORK1(k) + 0.129573 ! Kd(305nm) = 0.437891*CHL + 0.129573 ! new based on Tedetti et al. (2007)
                 KPARdz = KPARdz * delta_z(k)*0.01 ! note MARBL units for delta_z are CENTIMETERS and lin reg is for METERS (annoying)
                 E_inhUVB = E_inhUVB * exp(-KPARdz) ! propagation of E*inh thru column

                 !KPARdz = 0.4*WORK1(k) + 0.8 ! UV-C RADIATION
                 KPARdz = 0.6*WORK1(k) + 0.4 !UV-C RADIATION, PARTIALLY BASED ON SMITH AND BAKER (1980) AND TEDETTI (2007) RELATIONSHIPS
                 KPARdz = kPARdz*delta_z(k)*0.01
                 E_inhUVC = E_inhUVC*exp(-KPARdz)
                
                 E_inh = E_inhUVA + E_inhUVB + E_inhUVC
                 ...
                 !uv_limitation = 1 / (1+E_inh) ! for all PFTs, calculate gamma = 1 / (1+E*inh)
                 if (E_model) then
                   uv_limitation = 1.0_r8/(1.0_r8+E_inh) ! this is the basic default, what we have been doing
                 else if (T_model) then
                   uv_limitation = 1.0_r8 ! default no inhibition unless E*inh is less than or equal to 1
                   if (E_inh>1.0_r8) then
                      uv_limitation = 1.0_r8/(E_inh)
                   end if
                 else if (Emax_model) then
                   if (E_inh .lt. Emax_value) then
                     uv_limitation = 1.0_r8 / (1+E_inh)
                    else  ! if E_inh is greater than a value, more harsh inhibition
                     uv_limitation = 1.0_r8 / (CVALUE*E_inh) 
                   end if
                 end if
                    
                 if (auto_ind==4 .AND. E_inh>0.05) then  ! if coccolithophores, add enhancement if PICPOC is low and E*inh is above threshold - level specific !
                   !uv_limitation = 1 / (1+E_inh)
                   if (PICPOC(auto_ind,k)<0.05) then ! if PICPOC<0.1, defining it as 'naked' and scaling gamma by (1.0/3.5).
                     uv_limitation = uv_limitation * gamma_scale ! based on 3.5x growth rate diff between naked vs shelled cocco in Xu et al. (2016)
                   else if (PICPOC(auto_ind,k)<1) then !x^(1/6) slope from 1.0/3.5 to 1.0 between PICPOC=0.1 and PICPOC=1.0
                     uv_limitation = uv_limitation*abs(PICPOC(auto_ind,k))**(1.0/6.0)
                   !else ! when PICPOC=>1, do not scale uv_limitation !
                   !  uv_limitation= 1 / (1+E_inh)                 
                   end if
                 end if
            end if
  ...
  end subroutine compute_autotroph_photosynthesis_UVABCinh
