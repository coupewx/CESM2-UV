module radxfr_cam
  ...
  integer, parameter  :: nintegrals = 23 + 13 ! COUPE added this after adding a ton 
  real(r8), protected, allocatable :: swgt(:,:) ! (nintegrals, nwave)
  character(len=24) :: sname(nintegrals)
...
contains
  subroutine radxfr_cam_readnl(nlfile)
    ...
    logical            :: radxfr_do_actinic_so2 = .false.
    ...
    namelist /photo_radxfr_opts/ radxfr_wavelength_grid_file, radxfr_input_data_root, \
      ...
      radxfr_do_actinic_co2, radxfr_do_actinic_so2, radxfr_do_actinic_h2o, radxfr_do_actinic_hbr, \
    ...
    call mpi_bcast(radxfr_do_actinic_so2,1, mpi_logical, masterprocid, mpicom, ierr)
    ...
    if (masterproc) then    
       ...
       write(iulog,*) 'radxfr_cam_readnl: radxfr_do_actinic_so2 = ', radxfr_do_actinic_so2
    ...
    do_actinic_so2 = radxfr_do_actinic_so2
  ...
  subroutine radxfr_cam_register
    ...
    integer :: uva_idx, uvb_idx,uvc_idx, par_idx !, ephyto1_idx, ephyto2_idx, ephyto4_idx, ephyto5_idx
    integer :: ephyto1UVA_idx,ephyto1UVB_idx,ephyto1UVC_idx
    integer :: ephyto4UVA_idx,ephyto4UVB_idx,ephyto4UVC_idx
    integer :: ephyto5UVA_idx,ephyto5UVB_idx,ephyto5UVC_idx
    integer :: ephyto6UVA_idx,ephyto6UVB_idx,ephyto6UVC_idx
    integer :: ephyto7UVA_idx,ephyto7UVB_idx,ephyto7UVC_idx
    integer :: plant71_idx ! PLANT71 
    ...
    if (do_sfc_export) then
      ...
      call pbuf_add_field('TUV_UVC', 'physpkg', dtype_r8, (/pcols/), uvc_idx)
      ...
      call pbuf_add_field('TUV_PHYTO92_UVA','physpkg',dtype_r8, (/pcols/), ephyto1UVA_idx) ! Cullen et al. (1992) Diatoms - Phaeodactylum sp. using E model
      call pbuf_add_field('TUV_PHYTO92_UVB','physpkg',dtype_r8, (/pcols/), ephyto1UVB_idx)
      call pbuf_add_field('TUV_PHYTO92_UVC','physpkg',dtype_r8, (/pcols/), ephyto1UVC_idx)
      ! Neale and Thomas (2017) Small Phytoplankton - Prochlorococcus using Emax model
      call pbuf_add_field('TUV_PHYTO17_UVA','physpkg',dtype_r8, (/pcols/),ephyto4UVA_idx) ! Neale and Thomas 2017
      call pbuf_add_field('TUV_PHYTO17_UVB','physpkg',dtype_r8, (/pcols/),ephyto4UVB_idx)
      call pbuf_add_field('TUV_PHYTO17_UVC','physpkg',dtype_r8, (/pcols/),ephyto4UVC_idx)
      ! Lorenzo et al. (2019) Coccolithophores - Emiliania huxleyi using T model
      call pbuf_add_field('TUV_PHYTO19_UVA','physpkg',dtype_r8, (/pcols/),ephyto5UVA_idx)
      call pbuf_add_field('TUV_PHYTO19_UVB','physpkg',dtype_r8, (/pcols/),ephyto5UVB_idx)
      call pbuf_add_field('TUV_PHYTO19_UVC','physpkg',dtype_r8, (/pcols/),ephyto5UVC_idx)
      ! Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
      call pbuf_add_field('TUV_PHYTO10_UVA','physpkg',dtype_r8, (/pcols/),ephyto6UVA_idx)
      call pbuf_add_field('TUV_PHYTO10_UVB','physpkg',dtype_r8, (/pcols/),ephyto6UVB_idx)
      call pbuf_add_field('TUV_PHYTO10_UVC','physpkg',dtype_r8, (/pcols/),ephyto6UVC_idx)
      ! Neale and Thomas (2014) for Small Phytoplankton - Synechococcus using Emax model 
      call pbuf_add_field('TUV_PHYTO14_UVA','physpkg',dtype_r8, (/pcols/),ephyto7UVA_idx)
      call pbuf_add_field('TUV_PHYTO14_UVB','physpkg',dtype_r8, (/pcols/),ephyto7UVB_idx)
      call pbuf_add_field('TUV_PHYTO14_UVC','physpkg',dtype_r8, (/pcols/),ephyto7UVC_idx)
      ! add plant function PLANT71
      call pbuf_add_field('TUV_PLANT71','physpkg',dtype_r8, (/pcols/),plant71_idx)
  ...
  subroutine radxfr_cam_init
   ...
   !   s = s * (1e6/6.022142E23)(w/1e9)/(6.626068E-34*2.99792458E8)
    ...
    j = j+1
    sname(j)='TUV_PHYTO17_UVA'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton (Neale and Thomas, 2017) UV-A',flag_xyfill=.true.)
    !do iw = 1, nwave
    !    swgt(j,iw) = 0._r8
    !    if (wc(iw) .lt. 280._r8) then ! handling UV-C radiation in unique way because BWFs do not explicitly consider it 
    !       swgt(j,iw) = 7.70713857e-5_r8 * ABS(280.0_r8 - wc(iw)) + 0.0020263275390579594_r8 ! slope of line   
    !    endif
    ...
    ! file to use: /glade/work/jcoupe/ImpactPaper/Neale2017_prochlorococcus_spline.csv
    if (masterproc) then
       !call getfil(trim(input_data_root)//'/'//trim('DATAS1/phaeo.bio'), efile, fid)
       !call getfil(trim('/glade/work/jcoupe/ImpactPaper/Neale2017_prochlorococcus_spline.csv'),efile,fid)
       call getfil(trim(input_data_root)//'/'//trim('DATAS1/COUPE/Neale2017_prochlorococcus_spline.csv'), efile, fid)
       kin = getunit()
       open(unit=kin,file=efile,status='old')
       ! n = 106
       n = 278 ! need to adjust this number when the .bio file changes to match the number of lines
       do i = 1, n
       ...
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO17.')
       endif
       ...
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to interpolate for TUV_PHYTO17.')
       endif
    end if
    ...
    j = j+1
    sname(j)='TUV_PHYTO17_UVB'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton (Neale and Thomas, 2017) UV-B',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-1,:)
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j = j+1
    sname(j)='TUV_PHYTO17_UVC'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton (Neale and Thomas, 2017) UV-C',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-2,:)
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    !do iw = 1, nwave
    !    if (wc(iw) .lt. 280._r8) then ! if it's uv-c radiation, clear out uv-a and uv-b < 280 nm
    !       swgt(j-2,iw) = 0.0_r8 ! this means clear out uva radiation
    !       swgt(j-1,iw) = 0._r8 ! this means clear out uvb radiation
    !    endif
    !    if (wc(iw) .gt. 280._r8) then ! if it's uv-a or uv-b radiation >280 nm
    !            if (wc(iw) .lt. 315._r8) then ! if it's uv-b radiation
    !                   swgt(j,iw) = 0._r8 ! weighting for uv-c radiation is here
    !                   swgt(j-2,iw) = 0._r8 ! weighting for uv-a radiation is set to zero 
    !            endif
    !            if (wc(iw) .lt. 400) then ! if it's uv-a radiation then filter out weights for uva and uvb radiation
    !                   swgt(j-1,iw) = 0._r8 ! weighting for uvb radiation 
    !                   swgt(j,iw) = 0._r8 ! weighting for uvc radiation
    !            endif
    !    endif
    !    !write(*,*) "TUV_PHYTO17_UVC: ",wc(iw), swgt(j,iw)
    !enddo
        ...
        if (wc(iw) .lt. 280._r8) then ! if it's uv-c radiation, clear out uv-a and uv-b < 280 nm
           swgt(j-2,iw) = 0.0_r8 ! this means clear out uva radiation
           swgt(j-1,iw) = 0._r8 ! this means clear out uvb radiation
           if (wc(iw) .lt. 200._r8) then ! cut off lower bound off uv-c radiation
              swgt(j,iw) = 0._r8 ! turn off uv-c radiation below 200 nm 
           endif
        endif
        if (wc(iw) .gt. 280._r8) then ! if it's uv-a or uv-b radiation >280 nm
                swgt(j,iw) = 0._r8 ! uv-c radiation weights are zero when wc is gt 280
        endif
        if (wc(iw) .lt. 315._r8) then ! if still uv-b range
                swgt(j-2,iw) = 0._r8 !filter out uv-a radiation
        endif
        if (wc(iw) .gt. 315._r8) then ! if greater than UV-B range, zero out uv-b radiation
                swgt(j-1,iw) = 0._r8 !uv-b radiation can be zeroed out
        endif
    enddo

    !swgt(j,:) = swgt(j,:) * 1000._r8 ! equation is in units (mW m^-2)^-1 and spectral irradiance is in units W m^-2 ... wait, does this make sense?
    !swgt(j-1,:) = swgt(j-1,:) * 1000._r8
    !swgt(j-2,:) = swgt(j-2,:) * 1000._r8
    call mpibcast(swgt(j-2,:), nwave, mpir8, 0, mpicom)
    call mpibcast(swgt(j-1,:), nwave, mpir8, 0, mpicom)
    call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    !coccolithophores, Lorenzo et al. (2019)
    ! from paper examining Emiliana Huxleyi
    j = j+1
    sname(j)='TUV_PHYTO19_UVA'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Coccolithophores (Lorenzo et al., 2019) UV-A',flag_xyfill=.true.)
    !do iw = 1, nwave
    !   swgt(j,iw) = 0._r8 ! by default set this to 0 
    !   if (wc(iw) .lt. 280._r8) then ! handling UV-C radiation in unique way
    !       swgt(j,iw) = 0.00019074795916480976* ABS(280.0_r8 - wc(iw)) + 0.0034194075421964953
    !   else if (wc(iw) .gt. 280._r8 .and. wc(iw) .lt. 320._r8) then !UV-B
    ...
    !   endif
    !   swgt(j,iw) = max(swgt(j,iw), 0._r8) * 1000._r8
    !end do
    !swgt(j,:) = swgt(j,:) * 1000._r8
! Read from file.
    if (masterproc) then
       ...
       call getfil(trim(input_data_root)//'/'//trim('DATAS1/COUPE/Lorenzo2019_cocco_spline.csv'),efile,fid)
       kin = getunit()
       open(unit=kin,file=efile,status='old')
       ! n = 106
       n = 300 ! need to adjust this number when the .bio file changes to match the number of lines
       do i = 1, n
          read(kin,*) idum, dum1, dum2, y1(i)
          x1(i) = (dum1+dum2)/2._r8
       enddo
       ...
       call addpnt(x1,y1,kdata,n,x1(1)*(1._r8-deltax),y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO19.')
       endif
       call addpnt(x1,y1,kdata,n,0._r8,y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO19.')
       ...
       call inter2(nwave,wl,swgt(j,:nwave),n,x1,y1,ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to interpolate for TUV_PHYTO19.')
       endif
    end if
    ...
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j = j+1
    sname(j)='TUV_PHYTO19_UVB'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Coccolithophores (Lorenzo et al., 2019) UV-B',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-1,:)! * 1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j = j+1
    sname(j)='TUV_PHYTO19_UVC'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Coccolithophores (Lorenzo et al., 2019) UV-C',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-2,:)!* 1000._r8 
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    !do iw = 1, nwave
    !    if (wc(iw) .lt. 280._r8) then ! if it's uv-c radiation, clear out uv-a and uv-b < 280 nm
    !       swgt(j-2,iw) = 0.0_r8 ! this means clear out uva radiation
    ...
    ! add new bwfs for peer revisions !TUV_PHYTO10_UVA
    ! ! Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
    j = j+1
    sname(j)='TUV_PHYTO10_UVA'
    call addfld(trim(sname(j)),horiz_only,'A','1','Coccolithophores (Guan and Gao, 2010) UV-A',flag_xyfill=.true.)
    ! add file and interpolate it file = '/glade/work/jcoupe/ImpactPaper/coccoguan2009_spline.csv'

    ! Read from file.
    if (masterproc) then
       !call getfil(trim(input_data_root)//'/'//trim('DATAS1/phaeo.bio'), efile, fid)
       !call getfil(trim('/glade/work/jcoupe/misc_files/phaeo_dataextended.bio'), efile, fid) !extended file to 280-400 nm
       call getfil(trim(input_data_root)//'/'//trim('DATAS1/COUPE/coccoguan2009_spline.csv'),efile,fid)
       !call getfil(trim('/glade/work/jcoupe/cases/MAA_tests/IMPACT/MAA2.1_WACCM6_POP2_CLM4_RTMEBM_CICE4_SGLC_f19_g16_BGC_ciso_IMPACT_UV_02b_UVC/phaeo_dataextended.bio'),efile,fid) ! added uv-c to file
       kin = getunit()
       open(unit=kin,file=efile,status='old')
       ! n = 106
       n = 199 ! need to adjust this number when the .bio file changes to match the number of lines
       do i = 1, n
          read(kin,*) idum, dum1, dum2, y1(i)
          x1(i) = (dum1+dum2)/2._r8
       enddo
       ...
       call addpnt(x1,y1,kdata,n,x1(1)*(1._r8-deltax),y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO10.')
       endif
       call addpnt(x1,y1,kdata,n,0._r8,y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO10.')
       ...
       call inter2(nwave,wl,swgt(j,:nwave),n,x1,y1,ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to interpolate for TUV_PHYTO10.')
       endif
    end if
    !do iw=1,nwave
    !   !swgt(j,iw) = 0._r8 ! by default this should be zero
    !   swgt(j,iw) = swgt(j,iw) * 1000._r8
    !end do
    swgt(j,:) = swgt(j,:) * 1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j=j+1
    sname(j)='TUV_PHYTO10_UVB'
    call addfld(trim(sname(j)), horiz_only,'A','1','Coccolithophores (Guan and Gao, 2010) UV-B',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-1,:)! * 1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j=j+1
    sname(j)='TUV_PHYTO10_UVC'
    call addfld(trim(sname(j)), horiz_only,'A','1','Coccolithophores (Guan and Gao, 2010) UV-B',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-2,:)! * 1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    ! now filter out different wavelength bands
    !do iw = 1, nwave
    !    if (wc(iw) .lt. 280._r8) then ! if it's uv-c radiation, clear out uv-a and uv-b < 280 nm
    !       swgt(j-2,iw) = 0.0_r8 ! this means clear out uva radiation
    !       swgt(j-1,iw) = 0._r8 ! this means clear out uvb radiation
    ...
    ! add another new bwf - Neale and Thomas (2014) for Small Phytoplankton - Synechococcus using Emax model 
    j=j+1
    sname(j)='TUV_PHYTO14_UVA'
    call addfld(trim(sname(j)),horiz_only,'A','1','Synechococcus (Neale Thomas 2014) UV-A',flag_xyfill=.true.)
    
    ! Read from file.
    if (masterproc) then
       !call getfil(trim(input_data_root)//'/'//trim('DATAS1/phaeo.bio'), efile, fid)
       !call getfil(trim('/glade/work/jcoupe/misc_files/phaeo_dataextended.bio'), efile, fid) !extended file to 280-400 nm
       !call getfil(trim('/glade/work/jcoupe/ImpactPaper/coccoguan2009_spline.csv'),efile,fid)
       !call getfil(trim('/glade/work/jcoupe/cases/MAA_tests/IMPACT/MAA2.1_WACCM6_POP2_CLM4_RTMEBM_CICE4_SGLC_f19_g16_BGC_ciso_IMPACT_UV_02b_UVC/phaeo_dataextended.bio'),efile,fid) ! added uv-c to file
       call getfil(trim(input_data_root)//'/'//trim('DATAS1/COUPE//NT2014_synechococcus_final.csv'),efile,fid) ! cubic spline here
       kin = getunit()
       open(unit=kin,file=efile,status='old')
       ! n = 106
       n = 199 ! need to adjust this number when the .bio file changes to match the number of lines
       ...
       call addpnt(x1,y1,kdata,n,x1(1)*(1._r8-deltax),y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO14.')
       endif
       call addpnt(x1,y1,kdata,n,0._r8,y1(1),ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to addpnt for TUV_PHYTO14.')
       ...
       call inter2(nwave,wl,swgt(j,:nwave),n,x1,y1,ierr)
       if (ierr .ne. 0) then
          call endrun('radxfr_cam_init - ERROR: unable to interpolate for TUV_PHYTO14.')
       endif
    end if
    !do iw=1,nwave
    !   !swgt(j,iw) = 0._r8 ! by default this should be zero
    ...
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j=j+1
    sname(j)='TUV_PHYTO14_UVB'
    call addfld(trim(sname(j)), horiz_only,'A','1','Synechococcus (Neale Thomas 2014) UV-B',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-1,:)!*1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    j=j+1
    sname(j)='TUV_PHYTO14_UVC'
    call addfld(trim(sname(j)), horiz_only,'A','1','Synechococcus (Neale Thomas 2014) UV-C',flag_xyfill=.true.)
    swgt(j,:) = swgt(j-2,:)!*1000._r8
    !call mpibcast(swgt(j,:), nwave, mpir8, 0, mpicom)
    ! now filter for different wavelength bands 
    !do iw = 1, nwave
    !    if (wc(iw) .lt. 280._r8) then ! if it's uv-c radiation, clear out uv-a and uv-b < 280 nm
    !       swgt(j-2,iw) = 0.0_r8 ! this means clear out uva radiation
    !       swgt(j-1,iw) = 0._r8 ! this means clear out uvb radiation
    ...
    j = j + 1
    sname(j) = 'TUV_PHYTO92_UVA'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton, phaeo (Cullen et al., 1992) UV-A', flag_xyfill=.true.)
    ...
    if (masterproc) then
       ...
       !call getfil(trim('/glade/work/jcoupe/misc_files/phaeo_dataextended.bio'), efile, fid) !extended file to 280-400 nm
       !call getfil(trim('/glade/work/jcoupe/cases/MAA_tests/IMPACT/MAA2.1_WACCM6_POP2_CLM4_RTMEBM_CICE4_SGLC_f19_g16_BGC_ciso_IMPACT_UV_02b_UVC/phaeo_dataextended.bio'),efile,fid) ! added uv-c to file
       call getfil(trim(input_data_root)//'/'//trim('DATAS1/COUPE/Cullen1992_phaeodactylum_spline.csv'),efile,fid) ! added uv-c to file
       kin = getunit()
       open(unit=kin,file=efile,status='old')
       ! n = 106
       n = 278 ! need to adjust this number when the .bio file changes to match the number of lines
    ...
    !j = j + 1
    !sname(j) = 'TUV_PHYTOPHAEO92_UVA'
    !call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton, phaeo (Cullen et al., 1992) UV-A', flag_xyfill=.true.)
    swgt(j,:) = swgt(j,:)*1000._r8
    swgt(j+1,:) = swgt(j,:)
    swgt(j+2,:) = swgt(j,:)
    j = j + 1
    sname(j) = 'TUV_PHYTO92_UVB'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton, phaeo (Cullen et al., 1992) UV-B', flag_xyfill=.true.)
    j = j + 1
    sname(j) = 'TUV_PHYTO92_UVC'
    call addfld(trim(sname(j)), horiz_only, 'A', '1', 'Phytoplankton, phaeo (Cullen et al., 1992) UV-C', flag_xyfill=.true.)
       
    ! now print out  uv-a, uv-b, and uv-c weights !
    !do iw=1,nwave
    !  if ( wc(iw) .lt. 280_r8) then ! use j
    !   write(*,*) "UVC BEFORE: ",wc(iw),swgt(j,iw)
    !  endif
    !  if ( wc(iw) .gt. 280_r8) then 
    !     if ( wc(iw) .lt. 315_r8) then
    !             write(*,*) "UVB BEFORE: ",wc(iw),swgt(j-1,iw)
    !     endif
    !     if ( wc(iw) .gt. 315_r8) then
    !             write(*,*) "UVA BEFORE: ",wc(iw),swgt(j-2,iw)
    !     endif
    !  endif 
    !end do
    ...
    !swgt(j-2,:) = swgt(12,:) ! replace with phyto17 uv-a to test if the issue is with this portion of code or later on 
    ...
    !do iw=1,nwave
    !  if ( wc(iw) .lt. 280_r8) then ! use j
    !   write(*,*) "UVC AFTER: ",wc(iw),swgt(j,iw)
    !  endif
    !  if ( wc(iw) .gt. 280_r8) then
    !     if ( wc(iw) .lt. 315_r8) then
    !             write(*,*) "UVB AFTER: ",wc(iw),swgt(j-1,iw)
    !     endif
    !     if ( wc(iw) .gt. 315_r8) then
    !             write(*,*) "UVA AFTER: ",wc(iw),swgt(j-2,iw)
    !     endif
    !  endif
    !end do
    ...
    ! resulting integral will be unitlessthe 
    swgt(j,:) = swgt(j,:) * 1000._r8
    !write(*,*) "what is j at this stage?",j
    ! now print out weights to see why it's getting messed up !
    !do iw=1,nwave
    !  if (wc(iw) .lt. 401._r8) then
    !       write(*,*) "TUV_PHYTOPRORO92: ",wc(iw),swgt(j,iw) 
    !  end if
    !end do
