...
contains
  ...
  subroutine getwetr(carma, igroup, rh, rdry, rwet, rhopdry, rhopwet, rc, h2o_mass, h2o_vp, temp)
    ...
    if (irhswell(igroup) == I_WTPCT_H2SO4) then
      ...
      den1 = 2.00151_f - 0.000974043_f * temp_loc ! density at 79 wt %
      den2 = 2.01703_f - 0.000988264_f * temp_loc ! density at 80 wt %
      drho_dwt = den2-den1 ! change in density for change in 1 wt %
      
      sig1 = 79.3556_f - 0.0267212_f * temp_loc ! surface tension at 79.432 wt %
      sig2 = 75.608_f  - 0.0269204_f * temp_loc ! surface tension at 85.9195 wt %      
      ...
      rkelvinH2O_a = 2._f * gwtmol(igash2so4) * sigkelv / (den1 * RGAS * temp_loc * rwet)     
      rkelvinH2O = exp(rkelvinH2O_a*rkelvinH2O_b)      

      if (shr_infnan_isnan(h2o_mass)) then
         h2o_mass_tmp = 0.0_f
      else
         h2o_mass_tmp = h2o_mass
      end if
     ...
     ! end if
      ...
      h2o_kelv = h2o_mass_tmp / rkelvinH2O
      wtpkelv = wtpct_tabaz(carma, temp_loc, h2o_kelv, h2o_vp, rc)
      rhopwet   = sulfate_density(carma, wtpkelv, temp_loc, rc)
