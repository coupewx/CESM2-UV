  ...
  subroutine atm_export( cam_out, a2x )
    ...
    do c=begchunk, endchunk
	  ...
	  a2x(index_a2x_Sa_dens   ,ig) = cam_out(c)%rho(i)
          ...
          ! TUV metrics
          a2x(index_a2x_Faxa_uva,ig)     = cam_out(c)%uva(i)  
          a2x(index_a2x_Faxa_uvb,ig)     = cam_out(c)%uvb(i)  
          a2x(index_a2x_Faxa_uvc,ig)     = cam_out(c)%uvc(i)
          a2x(index_a2x_Faxa_par,ig)     = cam_out(c)%par(i)
          !a2x(index_a2x_Faxa_par_fake,ig)= cam_out(c)%par_fake(i) !%netsw(i)!cam_out(c)%par_fake(i)
          !a2x(index_a2x_Sa_ephyto1,ig)   = cam_out(c)%ephyto1(i)  
          !a2x(index_a2x_Sa_ephyto2,ig)   = cam_out(c)%ephyto2(i)  
          !a2x(index_a2x_Sa_ephyto3,ig)   = cam_out(c)%ephyto3(i)  
          !a2x(index_a2x_Sa_ephyto4,ig)   = cam_out(c)%ephyto4(i)
          !a2x(index_a2x_Sa_ephyto5,ig)   = cam_out(c)%ephyto5(i)

          ! ephyto1 is Cullen et al. (1992) Diatoms - Phaeodactylum sp. using E model
          a2x(index_a2x_Sa_ephyto1UVA,ig) = cam_out(c)%ephyto1UVA(i) ! 315 nm to 400 nm 
          a2x(index_a2x_Sa_ephyto1UVB,ig) = cam_out(c)%ephyto1UVB(i) ! 280 nm to 315 nm
          a2x(index_a2x_Sa_ephyto1UVC,ig) = cam_out(c)%ephyto1UVC(i) ! 100 nm to 280 nm

          ! ephyto4 is Neale and Thomas (2017) Small Phytoplankton - Prochlorococcus using Emax model (thresholds reported in paper)
          a2x(index_a2x_Sa_ephyto4UVA,ig) = cam_out(c)%ephyto4UVA(i) ! 315 nm to 400 nm 
          a2x(index_a2x_Sa_ephyto4UVB,ig) = cam_out(c)%ephyto4UVB(i) ! 280 nm to 315 nm
          a2x(index_a2x_Sa_ephyto4UVC,ig) = cam_out(c)%ephyto4UVC(i) ! 100 nm to 280 nm

          ! ephyto5 is Lorenzo et al. (2019) Coccolithophores - Emiliania huxleyi using T model
          a2x(index_a2x_Sa_ephyto5UVA,ig) = cam_out(c)%ephyto5UVA(i) ! 315 nm to 400 nm 
          a2x(index_a2x_Sa_ephyto5UVB,ig) = cam_out(c)%ephyto5UVB(i) ! 280 nm to 315 nm
          a2x(index_a2x_Sa_ephyto5UVC,ig) = cam_out(c)%ephyto5UVC(i) ! 100 nm to 280 nm

          ! ephyto6 is Guan and Gao (2009) Coccolithophores - Emiliania huxleyi using E model
          ! note this study finds uncalcified coccolithophores more impacted by UV radiation
          !a2x(index_a2x_Sa_ephyto6UVA,ig) = cam_out(c)%ephyto6UVA(i) ! 315 nm to 400 nm 
          !a2x(index_a2x_Sa_ephyto6UVB,ig) = cam_out(c)%ephyto6UVB(i) ! 280 nm to 315 nm
          !a2x(index_a2x_Sa_ephyto6UVC,ig) = cam_out(c)%ephyto6UVC(i) ! 100 nm to 280 nm 

          ! ephyto7 is Neale and Thomas (2017) for Small Phytoplankton - Synechococcus using Emax model (thresholds reported in
          ! paper
          a2x(index_a2x_Sa_ephyto7UVA,ig) = cam_out(c)%ephyto7UVA(i) ! 315 nm to 400 nm 
          a2x(index_a2x_Sa_ephyto7UVB,ig) = cam_out(c)%ephyto7UVB(i) ! 280 nm to 315 nm
          a2x(index_a2x_Sa_ephyto7UVC,ig) = cam_out(c)%ephyto7UVC(i) ! 100 nm to 280 nm

          a2x(index_a2x_Sa_plant71,ig) = cam_out(c)%plant71(i)
