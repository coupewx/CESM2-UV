...
!
  if (gas_wetdep_do_aq_nox .and. ((no_ndx == 0) .or. (no2_ndx == 0))) then
  ...
  if ( debug ) then
    ...
    print '(a,i4)','no_ndx',no_ndx
    print '(a,i4)','no2_ndx',no2_ndx
  end if
!
...
!
    kk = pver - k + 1
!
! CGB - Heff can get very large when extrapolated to very cold temperatures
! (e.g. 120 K) that can sometimes appear in WACCM. This can cause Heff
! values that are too big to output in single precision history files and
! thus can cause the model to crash. For wet deposition, there is no
! reason to have Heff values below TMIX as at colder temperatures Heff is not
! used in DISGAS and WASHGAS is not called for snow. Thus limit the minimum
! temperature to TFROZ for Heff calculations.
    do i = 1, ncol
      wrk(i) = (t0-max(TFROZ, tfld(i,kk)))/(t0*max(TFROZ, tfld(i,kk)))
    end do
!
    do m=1,gas_wetdep_cnt
!
...
!
    ...
    if (gas_wetdep_do_aq_nox) then
      ...
      do i = 1, ncol
        ...
        pno(i,k) = (trc_mass(i,k,no_ndx)/mass_in_layer(i,k))*(mwdry/mol_weight(no_ndx))*(p(i,k)/pstd_hpa)
        pno2(i,k) = (trc_mass(i,k,no2_ndx)/mass_in_layer(i,k))*(mwdry/mol_weight(no2_ndx))*(p(i,k)/pstd_hpa)
  
        ! Calculate aqueous reaction rates
        r1(i,k) = k1 * (heff(i,k,no2_ndx)**2) * (pno2(i,k)**2) * tc1
        r2(i,k) = k2 * heff(i,k,no_ndx) * pno(i,k) * heff(i,k,no2_ndx) * pno2(i,k) * tc1
        ...
        if (temp(i,k) .gt. TMIX) then
          ...
          heff(i,k,no_ndx) = heff(i,k,no_ndx) + (r2(i,k) / pno(i,k))
          heff(i,k,no2_ndx) = heff(i,k,no2_ndx) + ((2._r8 * r1(i,k) + r2(i,k)) / pno2(i,k))
        end if  
      end do
    end if
...
! CGB - Talked with DEK and he can't remember why this is was added and did a test
! that showed it did not make a significant difference. Would be better not to have
! this arbitrary criteria, but if it is needed it should be switched to the chemical
! tropopause. For now I have removed it.
...
!        FWASH = (RWASH*HSTAR*29.e-6_r8*PR)/(QM*BOXF)
        FWASH = (HSTAR*(1._r8/QM)*(mwdry)*(PR/pstd_hpa))*(RWASH/BOXF) / 1.0e3_r8
