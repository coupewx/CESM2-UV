...
module carma_intr
  ...
  ! Gas Statistics
  integer, parameter             :: NGSDIAGS         = 7          ! Number of gas diagnostics ...
  ...
  integer, parameter             :: GSDIAGS_GS       = 6          ! growth scaling factor
...
contains
  ...
  subroutine carma_register
    ...
    use physconst,    only: gravit, p_rearth=>rearth, mwdry, mwh2o, mwco2, mwso2, mwo3
...
!    do_wave_emit(nlwbands+1:) = .FALSE.
    ...
    if (masterproc) then
      ...
      write(LUNOPRT,*) '  carma_grow_limit_retries = ', carma_grow_limit_retries
      write(LUNOPRT,*) '  carma_grow_limit_scale   = ', carma_grow_limit_scale
    ...
    call CARMA_Initialize(carma, &
                          ...
                          do_grow_limit = carma_do_grow_limit, &
                          do_oxid       = carma_do_oxid, &
                          ...
                          glim_retries  = carma_grow_limit_retries, &
                          glim_scale    = carma_grow_limit_scale, &
    ...
    if ((masterproc) .and. (initial_run) .and. (radiation_scheme == "rrtmg") .and. (carma_do_optics)) then
      call CARMA_CreateOpticsFile(carma, rc)
       ...
       call CARMA_CreateMixOpticsFile(carma, rc)
       if (rc < 0) call endrun('carma_register::carma_CreateMixOpticsFiles failed.')
    end if
...
2   format(i6,4x,3(1pe11.3,4x))
    ...
    do igroup = 1, NGROUP
      ...
      call addfld(trim(sname)//'EM', (/ 'lev' /), 'A', 'kg/kg/s',trim(sname) // ' emissions')
      call addfld(trim(sname)//'SW', horiz_only, 'A', 'kg/m2/s', trim(sname) // ' wet deposition flux at surface')
      call addfld(trim(sname)//'DD', horiz_only, 'A', 'kg/m2/s', trim(sname) // ' dry deposition flux at surface')
      ...
      if (history_carma) then
         ...
         call add_default(trim(sname)//'EM', 1, ' ')
         call add_default(trim(sname)//'SW', 1, ' ')
         call add_default(trim(sname)//'DD', 1, ' ')
    ...
    do igas = 1, NGAS
     ...
     call addfld(trim(cnst_name(icnst))//'SF', horiz_only, 'A', 'kg/kg/s ', &
           trim(cnst_name(icnst)) // ' surface emission')
      call addfld(trim(cnst_name(icnst))//'EM', (/ 'lev' /), 'A', 'kg/kg/s ', &
           trim(cnst_name(icnst)) // ' emission')
      call addfld(trim(cnst_name(icnst))//'GS', horiz_only, 'A', 'fraction', &
           trim(cnst_name(icnst)) // ' growth limit scaling factor')
      
      if (history_carma) then
         call add_default(trim(cnst_name(icnst))//'SI', 1, ' ')
         call add_default(trim(cnst_name(icnst))//'SL', 1, ' ')
         call add_default(trim(cnst_name(icnst))//'EM', 1, ' ')
      end if
    end do
    ...
    ! Add fields for sulfate radiation
    if (carma_rad_feedback) then
      call addfld('CRRADREFF', (/ 'lev' /), 'A',  'm', 'carma reff for radiation')
      call addfld('CRRADMMR', (/ 'lev' /), 'A',   'kg/kg', 'carma mmr for radiation')
    end if
    
     ! Add the heat emissions.
    call addfld('HEATEM', (/ 'lev' /), 'A', 'J/kg/s', 'emissions of dry static energy')
  ...
  subroutine carma_timestep_tend(state, cam_in, cam_out, ptend, dt, pbuf, dlf, rliq, prec_str, snow_str, &
    ...
    real(r8)              :: grow_scale(pver)                       ! growth limit scaling factor
    ...
    do icol = 1, state_loc%ncol
      ...
      do igas = 1, NGAS
        ...
        call CARMASTATE_GetGas(cstate, igas, newstate(:), rc, satice=satice, satliq=satliq, &
         eqice=eqice, eqliq=eqliq, wtpct=wtpct, grow_scale=grow_scale)
        ...
        gsdiags(icol, :, igas, GSDIAGS_GS) = grow_scale(:)
...
!$OMP END CRITICAL
    ...
    call carma_output_diagnostics(pbuf, state_loc, ptend, gpdiags, sbdiags, gsdiags, spdiags, bndiags)
  ...
  !! @author  Chuck Bardeen
  !! @version May-2009
  subroutine carma_output_diagnostics(pbuf, state, ptend, gpdiags, sbdiags, gsdiags, spdiags, bndiags)
    ...
    logical           :: is_cloud  ! is the group a cloud?
    logical           :: do_drydep ! is dry deposition enabled?
    real(r8)          :: totdd(pcols) ! total dry deposition
    ...
    do igroup = 1, NGROUP 
      ...
      if (do_drydep) then
        ...
        do ibin = 1, NBIN
          ...
          totdd(:) = totdd(:) + sbdiags(:, ibin, ienconc, SBDIAGS_DD)
        end do
        call outfld(trim(sname)//'DD', totdd(:), pcols, lchnk)
      end if
    end do
    ...
    do igas = 1, NGAS
      ...
      call outfld(trim(cnst_name(icnst))//'GS', gsdiags(:, :, igas, GSDIAGS_GS), pcols, lchnk) 
    end do
    ...
    ! Output the effective radius and mass used for the sulfate radiation
    ! calculation.
    !
    ! NOTE: This is using a mechanism that was put in for prescribing
    ! volcanic aerosols, so it can not be combined with them.
    if (carma_rad_feedback) then
      call pbuf_get_field(pbuf, pbuf_get_index("VOLC_RAD_GEOM"), reffsulf_ptr)
      call outfld('CRRADREFF', reffsulf_ptr, pcols, lchnk)
      
      call pbuf_get_field(pbuf, pbuf_get_index("VOLC_MMR"), mmrsulf_ptr)
      call outfld('CRRADMMR', mmrsulf_ptr, pcols, lchnk)
    end if
  ...
  subroutine carma_emission_tend (state, ptend, cam_in, dt)
    ...
    integer      :: maxbin                  ! last prognostic bin
    integer      :: rc                      ! CARMA return code
    real(r8)     :: totem(pcols, pver)      ! total emission
    integer      :: ienconc
    character(len=8) :: sname               ! short (CAM) name
    integer      :: igas                    ! gas index
    ...
    ! Initialize the output tendency structure.
    call physics_ptend_init(ptend,state%psetcols, 'CARMA (emission)', ls=carma_do_emit_heat, lq=lq_carma)
    ...
    if (carma_do_emit_part) then
      do ielem = 1, NELEM
        call CARMAELEMENT_Get(carma, ielem, rc, igroup=igroup)
        if (rc < 0) call endrun('carma_drydep_tend::CARMAELEMENT_Get failed.')
      
        call CARMAGROUP_Get(carma, igroup, rc, cnsttype=cnsttype, maxbin=maxbin, ienconc=ienconc, shortname=sname)
        ...
        if (cnsttype == I_CNSTTYPE_PROGNOSTIC) then
          ...
          do ibin = 1, NBIN
            ...
            if (ibin <= maxbin) then
              ...
              cam_in%cflx(:ncol, icnst) = cam_in%cflx(:ncol, icnst) + surfaceFlux(:ncol)
              call outfld(trim(cnst_name(icnst))//'SF', surfaceFlux(:ncol), ncol, lchnk)
            
              ! For emissions into the atmosphere, put the emission here.
              ptend%q(:ncol, :pver, icnst) = ptend%q(:ncol, :pver, icnst) + tendency(:ncol, :pver)
              call outfld(trim(cnst_name(icnst))//'EM', tendency(:ncol, :), ncol, lchnk)
            
              if (ielem == ienconc) then
                totem(:ncol,:pver) = totem(:ncol,:pver) + ptend%q(:ncol, :pver, icnst)
              end if
            end if
          enddo
        
          if (ielem == ienconc) then
            call outfld(trim(sname)//'EM', totem(:ncol, :), ncol, lchnk)
          end if
    ...
    ! Emission for gases.
    if (carma_do_emit_gas) then
      do igas = 1, NGAS
        icnst = icnst4gas(igas)

        call CARMA_EmitGas(carma, igas, icnst, dt, state, cam_in, tendency, surfaceFlux, rc)
        if (rc < 0) call endrun('carma_emission_tend::CARMA_EmitGas failed.')
    ...
    ! Emission of heat.
    if (carma_do_emit_heat) then
    
      call CARMA_EmitHeat(carma, dt, state, cam_in, tendency, rc)
      if (rc < 0) call endrun('carma_emission_tend::CARMA_EmitHeat failed.')
    
      ptend%s(:ncol, :pver) = ptend%s(:ncol, :pver) + tendency(:ncol, :pver)
      call outfld('HEATEM', tendency(:ncol, :), ncol, lchnk)
    end if
   ...
   cmfdqr(:ncol,:) = rprddp(:ncol,:)  + rprdsh(:ncol,:)
    ...
    do ielem = 1, NELEM
      ...
      call CARMAGROUP_Get(carma, igroup, rc, cnsttype=cnsttype, do_wetdep=do_wetdep, &
        solfac=solfac, scavcoef=scavcoef, maxbin=maxbin, ienconc=ienconc, shortname=sname)
      ...
      if ((do_wetdep) .and. (cnsttype == I_CNSTTYPE_PROGNOSTIC)) then
        ...
        do ibin = 1, NBIN
          ...
          if (ibin <= maxbin) then
            ...
            if (ielem == ienconc) then
              totsw(:ncol) = totsw(:ncol) + sflx(:ncol) 
            end if
        ...
        if (ielem == ienconc) then
          call outfld(trim(sname)//'SW', totsw, pcols, lchnk)
        end if
  ...
  subroutine CARMA_CreateOpticsFile(carma, rc)
    ...
    do igroup = 1, NGROUP
      ...
      call CARMAGROUP_Get(carma, igroup, rc, do_mie=do_mie, name=name, shortname=shortname, r=r, &
                          ...
                          ienconc=ienconc, cnsttype=cnsttype, maxbin=maxbin, is_mixed=ismixed)
      if (rc < 0) call endrun('carma_CreateOpticsFile::CARMAGROUP_Get failed.')
      
      ! Are we supposed to do the mie calculation for this group?
      if ((do_mie) .and. (cnsttype == I_CNSTTYPE_PROGNOSTIC) .and. (.not. ismixed)) then
...
!write(carma%f_LUNOPRT,*) "CARMA mie calc:  start  ", igroup, ibin, iwave, carma%f_wave(iwave), carma%f_group(igroup)%f_nmon(ibin)
                ...
                call mie(carma, &
                         ...
                         rcore, &
                         carma%f_group(igroup)%f_refidx_core(iwave), &
                         Qext, &
                         Qsca, &
                         asym, &
                         rc)
...
!write(carma%f_LUNOPRT,*) "CARMA mie calc:  done  ", Qext, Qsca, asym
  ...
  !! This routine creates files containing optical properties for radiatively
  !! active particle types that are mixtures. These optical properties are used
  !! by the RRTMG radiation code to include the impact of CARMA particles in
  !! the radiative transfer calculation.
  !!
  !! For mixtures, a file need to be created for each composition type that has
  !! things like density and refractive index. Another file needs to be created
  !! that is a lookup table for the optical properties based up different
  !! mixtures of the components. The actual calculation of the optical
  !! properties will be in coreshell_aer_opt.F90
  !!
  !! NOTE: This routine is currently set up for dry 2 component core/shell
  !! particles. In the future in could be enhanced for other mixtures.
  subroutine CARMA_CreateMixOpticsFile(carma, rc)
    use radconstants, only : nswbands, nlwbands
    use wrap_nf
    use wetr, only         : getwetr
    ...
    integer                             :: igroup, ibin, iwave, irh, ielem, ielem2
    integer                             :: irhswell
    integer                             :: ienconc
    real(kind=f)                        :: rho(NBIN), rhopwet, rhoelem(NBIN)
    real(kind=f)                        :: r(NBIN), rmass(NBIN), rlow(NBIN), rup(NBIN)
    real(kind=f)                        :: wave(NWAVE)
    complex(kind=f)                     :: refidx(NWAVE), refidx_core(NWAVE), refidx_elem(NWAVE)
    character(len=CARMA_NAME_LEN)       :: name
    character(len=CARMA_SHORT_NAME_LEN) :: shortname, elemsname
    logical                             :: do_mie
    integer                             :: fid
    integer                             :: cfdim, lwdim, swdim
    integer                             :: cfvar, lwvar, swvar
    integer                             :: abs_lw_var
    integer                             :: ext_sw_var, abs_sw_var, asm_sw_var
    integer                             :: sw_cf_var, lw_cf_var
    integer                             :: refrlvar, refilvar, refrsvar, refisvar
    integer                             :: omdim, andim, namedim 
    integer                             :: omvar, anvar, namevar
    integer                             :: ncfvar, massvar, rtotvar, rcorvar
    integer                             :: dimids(2)
    integer                             :: denvar, slogvar, dryrvar, rminvar, rmaxvar, hygrovar, ntmvar
    real(kind=f)                        :: abs_lw(NFRAC, nlwbands)
    real(kind=f)                        :: ext_sw(NFRAC, nswbands)
    real(kind=f)                        :: abs_sw(NFRAC, nswbands)
    real(kind=f)                        :: asm_sw(NFRAC, nswbands)
    character(len=8)                    :: c_name                   ! constituent name  
    character(len=32)                   :: aer_name                 ! long enough for both aername and name  
    character(len=255)                  :: filepath
    real(kind=f)                        :: rwet
    ...
    real(kind=f)                        :: Qsca
    real(kind=f)                        :: asym
    real(kind=f)                        :: rcore(NFRAC), rtotal(NFRAC), volcore, voltotal, rhocore, rhopart
    integer                             :: start_text(2), count_text(2)
    integer                             :: sw_r_refidx_var, sw_i_refidx_var, lw_r_refidx_var, lw_i_refidx_var
    integer                             :: cnsttype               ! constituent type
    integer                             :: maxbin                 ! last prognostic bin
    ...
    real(kind=f), parameter             :: corefrac(NFRAC) = (/ 0.0_r8, 0.01_r8, 0.02_r8, 0.04_r8, &
                                             0.08_r8, 0.1_r8, 0.2_r8, 0.3_r8, 0.4_r8, 0.5_r8, 0.6_r8, &
                                             0.7_r8, 0.8_r8, 0.9_r8, 1._r8 /)
    ...
    ! Get the wavelength structure.
    call CARMA_GET(carma, rc, wave=wave, do_print=do_print, LUNOPRT=LUNOPRT)
    if (rc < 0) call endrun('carma_CreateMixOpticsFile::CARMA_Get failed.')
    ...
    do igroup = 1, NGROUP
      ...
      call CARMAGROUP_Get(carma, igroup, rc, do_mie=do_mie, name=name, shortname=shortname, r=r, &
                          rlow=rlow, rup=rup, rmass=rmass, refidx=refidx, irhswell=irhswell, &
                          ienconc=ienconc, cnsttype=cnsttype, maxbin=maxbin, is_mixed=ismixed, &
                          ncore=ncore, icorelem=icorelem, refidx_core=refidx_core, rhocore=rhocore, &
                          rhopart=rhopart)
      if (rc < 0) call endrun('carma_CreateMixOpticsFile::CARMAGROUP_Get failed.')
      
      ! Are we supposed to do the mie calculation for this group?
      if ((do_mie) .and. (cnsttype == I_CNSTTYPE_PROGNOSTIC) .and. ismixed) then
      
        ! Only support 2 component mixtures at the moment.
        if (ncore .ne. 1) call endrun('carma_CreateMixOpticsFile::CARMAGROUP_Get Only two component mixtures supported.')
        
        ! For each component, create a aerosol properties file.
        !
        ! NOTE: To do this right, need to move refractive index to the element.
        do ielem = 1, ncore+1
        
          if (ielem .eq. ncore+1) then
          	ielem2 = ienconc
          	refidx_elem = refidx
          else
            ielem2 = icorelem(ielem)
          	refidx_elem = refidx_core
          end if
        
          ! Get the necessary group properties.
          call CARMAELEMENT_Get(carma, ielem2, rc, rho=rhoelem, shortname=elemsname)
          if (rc < 0) call endrun('carma_CreateMixOpticsFile::CARMAGROUP_Get failed.')

            ! The fields in the file need to be:
            !
            !   lw_band
            !   sw_band
            !   refindex_real_aer_sw
            !   refindex_im_aer_sw
            !   refindex_real_aer_lw
            !   refindex_im_aer_lw
            !   density_aer
            !   hygro_aer           

            filepath = trim(carma_model) // '_' // trim(elemsname) // '_rrtmg.nc'
            
            if (do_print) write(LUNOPRT,*) 'Creating CARMA mix optics properties file ... ', trim(filepath)
            ...
            write(LUNOPRT,*) "Defined lw_band, and sw_band dims."

            dimids(1) = lwdim
            call wrap_def_var(fid, 'lw_band', NF90_DOUBLE, 1, dimids(1:1), lwvar)
            call wrap_def_var(fid, 'refindex_real_aer_lw', NF90_DOUBLE, 1, dimids(1:1), refrlvar)
            call wrap_def_var(fid, 'refindex_im_aer_lw', NF90_DOUBLE, 1, dimids(1:1), refilvar)
            
            dimids(1) = swdim
            call wrap_def_var(fid, 'sw_band', NF90_DOUBLE, 1, dimids(1:1), swvar)
            call wrap_def_var(fid, 'refindex_real_aer_sw', NF90_DOUBLE, 1, dimids(1:1), refrsvar)
            call wrap_def_var(fid, 'refindex_im_aer_sw', NF90_DOUBLE, 1, dimids(1:1), refisvar)
            ...
            call wrap_put_att_text(fid, lwvar, 'long_name', 'longwave bands')
            call wrap_put_att_text(fid, swvar, 'long_name', 'shortwave bands')
            call wrap_put_att_text(fid, refrlvar, 'long_name', 'real refractive index of water - longwave')
            call wrap_put_att_text(fid, refilvar, 'long_name', 'imaginary refractive index of water - longwave')
            call wrap_put_att_text(fid, refrsvar, 'long_name', 'real refractive index of water - shortwave')
            call wrap_put_att_text(fid, refisvar, 'long_name', 'imaginary refractive index of water - shortwave')

            write(LUNOPRT,*) "Defined lw_band, and sw_band vars."
            ...
            ! Write of the refractive indicies
            call wrap_put_var_realx(fid, refrlvar, real(refidx_elem(:nlwbands)))
            call wrap_put_var_realx(fid, refilvar, aimag(refidx_elem(:nlwbands)))
            call wrap_put_var_realx(fid, refrsvar, real(refidx_elem(:nswbands)))
            call wrap_put_var_realx(fid, refisvar, aimag(refidx_elem(:nswbands)))
                          
            start_text(1) = 1
            count_text(1) = len('sectional_props                 ')
            call wrap_put_vara_text(fid, omvar, start_text, count_text, (/ 'sectional_props                 ' /))
            
            call wrap_put_var_realx(fid, denvar,   (/ rhoelem(1) * 1e-3_f / 1e-6_f /))
            call wrap_put_var_realx(fid, hygrovar, (/ 0._f /))
        ...
        do ibin = 1, NBIN
          ...
          if (ibin <= maxbin) then
            ...
            !
            !   opticsmethod_len = 32
            !   opticsmethod = "sectional"
            !   lw_band
            !   sw_band
            !   corefrac_number
            !   extpsw2(nfrac,nswbands)
            !   abspsw2(nfrac,nswbands)
            !   asmpsw2(nfrac,nswbands)
            !   absplw2(nfrac,nlwbands)
            !   corefracsw(nfrac)
            !   corefraclw(nfrac)
            ...
            if (do_print) write(LUNOPRT,*) 'Creating CARMA mix optics lookup table file ... ', trim(filepath)

            ! Create the file.
            call wrap_create(filepath, NF90_CLOBBER, fid)

            ! Define the dimensions: rh, lwbands, swbands
            call wrap_def_dim(fid, 'corefrac',  NFRAC,  cfdim)
            call wrap_def_dim(fid, 'lw_band', nlwbands, lwdim)
            call wrap_def_dim(fid, 'sw_band', nswbands, swdim)

            write(LUNOPRT,*) "Defined corefrac, lw_band, and sw_band dims."

            dimids(1) = cfdim
            call wrap_def_var(fid, 'corefrac',  NF90_DOUBLE, 1, dimids(1:1), cfvar)
            call wrap_def_var(fid, 'rtotal',  NF90_DOUBLE, 1, dimids(1:1), rtotvar)
            call wrap_def_var(fid, 'rcore',  NF90_DOUBLE, 1, dimids(1:1), rcorvar)
            ...
            call wrap_put_att_text(fid, cfvar, 'units', 'fraction') 
            call wrap_put_att_text(fid, rtotvar, 'units', 'm') 
            call wrap_put_att_text(fid, rcorvar, 'units', 'm') 
            call wrap_put_att_text(fid, lwvar, 'units', 'm') 
            call wrap_put_att_text(fid, swvar, 'units', 'm') 
  
            call wrap_put_att_text(fid, cfvar, 'long_name', 'core fraction')
            call wrap_put_att_text(fid, rtotvar, 'long_name', 'total radius')
            call wrap_put_att_text(fid, rcorvar, 'long_name', 'core radius')
            call wrap_put_att_text(fid, lwvar, 'long_name', 'longwave bands')
            call wrap_put_att_text(fid, swvar, 'long_name', 'shortwave bands')

            write(LUNOPRT,*) "Defined corefracr, lw_band, and sw_band vars."

            ! Define the variables: abs_lw, ext_sw, ssa_sw, asm_sw
            dimids(1) = cfdim
            dimids(2) = lwdim
            call wrap_def_var(fid, 'absplw2', NF90_DOUBLE, 2, dimids, abs_lw_var)
            ...
            dimids(1) = cfdim
            dimids(2) = swdim
            call wrap_def_var(fid, 'extpsw2', NF90_DOUBLE, 2, dimids, ext_sw_var)
            call wrap_def_var(fid, 'abspsw2', NF90_DOUBLE, 2, dimids, abs_sw_var)
            call wrap_def_var(fid, 'asmpsw2', NF90_DOUBLE, 2, dimids, asm_sw_var)
            
            call wrap_put_att_text(fid, abs_sw_var, 'units', 'meter^2 kilogram^-1') 
            call wrap_put_att_text(fid, ext_sw_var, 'units', 'meter^2 kilogram^-1') 
            call wrap_put_att_text(fid, asm_sw_var, 'units', '-') 

            write(LUNOPRT,*) "Defined extpsw2, abspsw2, and asmpsw2."
            ...
            call wrap_def_var(fid, 'corefrac_number', NF90_INT, 0, dimids(1:0), ncfvar)
            call wrap_put_att_text(fid, ncfvar,   'units', '') 
            call wrap_put_att_text(fid, ncfvar,   'long_name', 'number of core fractions') 

            call wrap_def_var(fid, 'binmass', NF90_DOUBLE, 0, dimids(1:0), massvar)
            call wrap_put_att_text(fid, massvar,   'units', 'kg') 
            call wrap_put_att_text(fid, massvar,   'long_name', 'mass of bin') 
            ...
            ! Write out the dimensions.
            call wrap_put_var_realx(fid, cfvar, corefrac)
            call wrap_put_var_realx(fid, massvar, (/ rmass(ibin) / 1e3_f /))
            call wrap_put_var_realx(fid, lwvar, wave(:nlwbands) * 1e-2_f)
            call wrap_put_var_realx(fid, swvar, wave(nlwbands+1:) * 1e-2_f)
                          
            start_text(1) = 1
            count_text(1) = len('sectional                       ')
            call wrap_put_vara_text(fid, omvar, start_text, count_text, (/ 'sectional                       ' /))
            
            call wrap_put_var_int(fid, ncfvar,   (/ NFRAC /))
           
            ! Iterate over a range of core fractions.
            do ifrac = 1, NFRAC
            
              ! Really need to determine the volume here for core and shell and then
              ! determine the respective radii,
              
              ! Determine the wet radius.
!              call getwetr(carma, igroup, mie_rh(irh), r(ibin), rwet, rho(ibin), rhopwet, rc)
!              if (rc < 0) call endrun('carma_CreateOpticsFile::wetr failed.')
              
              !   rcore = (fraccore * rhopart) / rhocore) ^ 1/3 * radius
!              rcore = ((corefrac(ifrac) * carma%f_group(igroup)%f_rhopart / carma%f_group(igroup)%f_rhocore) ** (1._r8/3._r8)) * rwet 
              volcore = corefrac(ifrac) * rmass(ibin) / rhocore
              rcore(ifrac) = (3._f / 4._f * volcore / PI ) ** (1._f / 3._f)
              voltotal = volcore + (1. - corefrac(ifrac)) * rmass(ibin) / rhopart
              rtotal(ifrac) = (3._f / 4._f * voltotal / PI ) ** (1._f / 3._f)
              ...
              do iwave = 1, NWAVE
                ...
                if (iwave <= nlwbands) then
                  ...
                  ! Longwave just needs absorption: abs_lw.
                  abs_lw(ifrac, iwave) = (Qext - Qsca) * PI * (rtotal(ifrac) * 1e-2_f)**2 / (rmass(ibin) * 1e-3_f)
                else
               
                  ! Shortwave needs extinction, single scattering albedo and asymmetry factor:
                  ! ext_sw, ssa_sw and asm_sw.
                  ext_sw(ifrac, iwave - nlwbands) = Qext * PI * ((rtotal(ifrac) * 1e-2_f)**2) / (rmass(ibin) * 1e-3_f)
                  abs_sw(ifrac, iwave - nlwbands) = (Qext - Qsca) * PI * ((rtotal(ifrac) * 1e-2_f)**2) / (rmass(ibin) * 1e-3_f)
                  asm_sw(ifrac, iwave - nlwbands) = asym
                end if
              end do
            end do

            call wrap_put_var_realx(fid, rtotvar, rtotal * 1e-2_f)
            call wrap_put_var_realx(fid, rcorvar, rcore * 1e-2_f)

            ! Write out the longwave fields.
            ret = nf90_put_var (fid, abs_lw_var, abs_lw(:, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateMixOpticsFile: error writing varid =', abs_lw_var
               call handle_error (ret)
            end if
            
            ! Write out the shortwave fields.
            ret = nf90_put_var (fid, ext_sw_var, ext_sw(:, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateMixOpticsFile: error writing varid =', ext_sw_var
               call handle_error (ret)
            end if
            ret = nf90_put_var (fid, abs_sw_var, abs_sw(:, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateMixOpticsFile: error writing varid =', abs_sw_var
               call handle_error (ret)
            end if
            ret = nf90_put_var (fid, asm_sw_var, asm_sw(:, :))
            if (ret/=NF90_NOERR) then
               write(iulog,*)'CARMA_CreateMIxOpticsFile: error writing varid =', asm_sw_var
               call handle_error (ret)
            end if
    ...
    return
  end subroutine CARMA_CreateMixOpticsFile   
